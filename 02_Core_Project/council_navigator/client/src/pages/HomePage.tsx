...
import { Building2, ChevronRight, Search, Activity, BarChart3, Settings as SettingsIcon, ArrowRight, Newspaper, } from "lucide-react";
interface StatsData {
      ...
      <header className="sticky top-0 z-40 bg-[var(--canvas)]/95 backdrop-blur border-b border-[var(--line)]">
        <div className="max-w-7xl mx-auto px-6 lg:px-10 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3.5">
            <div className="bg-white text-black p-2 rounded-md" aria-hidden="true">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 pt-14 pb-20 kg-fade-in">
        ...
        <section className="mb-14">
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={() => onNavigate("broadcast", { meetingId: 9 })} className="zs-cta-accent group inline-flex items-center gap-2.5 px-5 py-3 text-[12px] uppercase tracking-widest" title="Sample broadcast: Kingman City Council, Apr 7, 2026">
              <Newspaper className="w-3.5 h-3.5"/>
              View Sample Broadcast
              <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform"/>
            </button>
            <button onClick={() => onNavigate("search")} className="zs-cta-primary group inline-flex items-center gap-2.5 px-5 py-3 text-[12px] uppercase tracking-widest">
              <Search className="w-3.5 h-3.5"/>
              Search Meetings
              <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform"/>
            </button>
            <button onClick={() => onNavigate("pipeline")} className="inline-flex items-center gap-2.5 px-5 py-3 bg-[var(--surface)] hover:bg-[var(--surface-3)] border border-[var(--line)] text-[var(--foreground)] rounded-md font-semibold uppercase tracking-widest text-[11px] transition-colors">
              <Activity className="w-3.5 h-3.5"/>
              Pipeline Monitor
            </button>
            <button onClick={() => onNavigate("dashboard")} className="inline-flex items-center gap-2.5 px-5 py-3 bg-[var(--surface)] hover:bg-[var(--surface-3)] border border-[var(--line)] text-[var(--foreground)] rounded-md font-semibold uppercase tracking-widest text-[11px] transition-colors">
              <BarChart3 className="w-3.5 h-3.5"/>
              Parser Dashboard
            </button>
            <button onClick={() => onNavigate("settings")} className="inline-flex items-center gap-2.5 px-5 py-3 bg-[var(--surface)] hover:bg-[var(--surface-3)] border border-[var(--line)] text-[var(--foreground)] rounded-md font-semibold uppercase tracking-widest text-[11px] transition-colors">
        ...
        ].map((stat, i) => (<div key={i} className="kg-card p-6 hover:bg-[var(--surface-3)]/60 transition-colors">
                <p className="text-3xl md:text-4xl font-light tabular-nums text-white">
                  {loading ? (<span className="text-muted-foreground/40">—</span>) : (<AnimatedNumber value={stat.value}/>)}
                </p>
        ...
        <section className="mb-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            return (<button key={cityName} onClick={() => onNavigate("city", {
                    cityName,
                    county: "Mohave County",
                    state: "Arizona",
                })} className={`group relative overflow-hidden rounded-xl p-5 text-left transition-all duration-200 border ${hasData
                  ...
                  {hasData ? (<p className="text-xs text-muted-foreground tabular-nums">
                      {meetingCount.toLocaleString()} meetings
                    </p>) : (<p className="text-xs text-muted-foreground/70 italic">
        ...
        {mohaveCities.length > 0 && (<section className="mb-16">
            <h3 className="kg-eyebrow mb-5">Most Active Mohave Cities</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {mohaveCities.slice(0, 8).map((city, idx) => (<button key={idx} onClick={() => onNavigate("city", {
                    cityName: city.city,
                    county: city.county,
                    state: "Arizona",
        ...
        <section>
          <div className="kg-card p-8 text-center">
            ...
            <h4 className="text-xl font-light text-white mb-3 tracking-wide">
              Mohave first. Arizona next. Every other state is an open seat.
            </h4>
            <p className="text-sm text-muted-foreground max-w-xl mx-auto leading-relaxed">
              Z-SPAN — Arizona starts narrow and proves the model in Mohave
              County before expanding to the rest of Arizona. Other states are
              run by their own operators, keeping the same neutrality and the
              same transparency.
            </p>
          </div>
        </section>
