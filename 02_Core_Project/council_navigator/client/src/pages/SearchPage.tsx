...
interface SearchPageProps {
  /** Optional seed query (V1-Polish-12). When set, the page mounts with
   *  this text in the box and runs the search once automatically — the
   *  TopBar search routes here via onNavigate("search", { query }). */
  initialQuery?: string;
}
...
export default function SearchPage({ initialQuery }: SearchPageProps = {}) {
  ...
  // Auto-run the seeded query once on mount (V1-Polish-12). searchQuery is
  // already initialized to initialQuery, so the first handleSearch closure
  // carries it. The ref-guard keeps it to a single fire even though
  // handleSearch is a dependency that changes as the user edits filters.
      ...
      <div className="kg-card p-6 mb-8">
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            placeholder="Search by meeting title, city, county, or location…"
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {datePresets.map(preset => (
            <button
              key={preset.label}
              onClick={preset.fn}
              className="px-3 py-1.5 rounded-md bg-[var(--canvas)] hover:bg-[var(--surface-3)] border border-[var(--line)] text-muted-foreground hover:text-foreground text-[11px] font-medium uppercase tracking-wider transition-colors"
            >
              {preset.label}
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2.5 rounded-md bg-[var(--canvas)] border border-[var(--line)] text-foreground/85 text-sm focus:outline-none focus:border-[var(--line-strong)] transition-colors appearance-none cursor-pointer"
            >
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2.5 rounded-md bg-[var(--canvas)] border border-[var(--line)] text-foreground/85 text-sm focus:outline-none focus:border-[var(--line-strong)] transition-colors"
            />
          </div>
          <div>
        ...
        <div className="flex gap-3">
          <button
            ...
            disabled={loading}
            className="flex-1 bg-white hover:bg-gray-200 text-black px-6 py-3 rounded-md font-semibold uppercase tracking-widest text-[11px] transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {loading ? (
          ...
          {(selectedCounty || dateFrom || dateTo || searchQuery) && (
            <button
              onClick={clearFilters}
              className="bg-[var(--canvas)] hover:bg-[var(--surface-3)] border border-[var(--line)] text-muted-foreground hover:text-foreground px-5 py-3 rounded-md font-semibold uppercase tracking-widest text-[11px] transition-colors flex items-center gap-2"
            >
      ...
      {hasSearched && (
        <div>
          <div className="kg-card p-5 mb-3 flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-2xl font-light text-white mt-1 tabular-nums">
                ...
                {searchTime > 0 && (
                  <span className="ml-3 text-xs text-muted-foreground tracking-wider uppercase">
                    · {searchTime}ms
                  </span>
                )}
              </p>
            ...
            {totalPages > 1 && (
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 rounded-md bg-[var(--canvas)] border border-[var(--line)] text-muted-foreground hover:text-foreground disabled:opacity-30 transition-colors"
                >
          ...
          {searchResults.length > 0 ? (
            <div className="kg-card overflow-hidden">
              <div className="divide-y divide-[var(--line)]">
                {searchResults.map((result, index) => (
                  <div
                    key={result.public_id ?? result.id ?? index}
                    className="p-5 hover:bg-[var(--surface-3)]/30 transition-colors"
                  >
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {result.agenda_url && (
                        <a
                          ...
                          className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[var(--canvas)] border border-[var(--line)] hover:bg-[var(--surface-3)] text-muted-foreground hover:text-foreground text-[11px] font-medium uppercase tracking-wider transition-colors"
                        >
                      ...
                      <button
                        onClick={() => handleDownloadICS(result)}
                        className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[var(--canvas)] border border-[var(--line)] hover:bg-[var(--surface-3)] text-muted-foreground hover:text-foreground text-[11px] font-medium uppercase tracking-wider transition-colors"
                      >
                        <Download className="w-3 h-3" /> .ics
                      </button>
                      <button
                        onClick={() => handleGoogleCalendar(result)}
                        className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[var(--canvas)] border border-[var(--line)] hover:bg-[var(--surface-3)] text-muted-foreground hover:text-foreground text-[11px] font-medium uppercase tracking-wider transition-colors"
                      >
            ...
            <div className="kg-card p-12 text-center">
          ...
          {totalPages > 1 && (
            <div className="mt-5 flex justify-center gap-2">
              <button
                onClick={() => handleSearch(currentPage - 1)}
                disabled={currentPage <= 1 || loading}
                className="px-4 py-2 rounded-md bg-[var(--surface-2)] border border-[var(--line)] text-muted-foreground hover:text-foreground disabled:opacity-30 transition-colors text-xs uppercase tracking-wider font-medium flex items-center gap-1.5"
              >
