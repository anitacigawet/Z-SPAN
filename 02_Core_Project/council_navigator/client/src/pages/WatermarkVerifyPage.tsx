/**
 * S-098 Phase 2 V0 — provenance verifier + educational landing page.
 *
 * Per operator framing: the ribbon's slight visibility IS the feature.
 * Users notice it on a Z-SPAN page → click it out of curiosity → land
 * here → see the prefilled verdict AND learn what the system is for.
 * That priming means when they later encounter suspicious Z-SPAN-
 * attributed content elsewhere on the internet, they already know to
 * come back here and verify it.
 *
 * Two entry paths:
 *   1. ?token=XXX direct landing from a ribbon click — show verdict,
 *      then introduce the system + the decode animation + the upload
 *      affordance for future off-site verification.
 *   2. Cold visit (no token) — same educational content + upload form
 *      front-and-center for users who arrived because they have a
 *      suspicious image to check.
 *
 * Small SVG decode animation walks the user through the block-to-token
 * mapping the way MD5-hash visualizations do — satisfying + educational.
 */
...
function tokenToBlocks(token: string): number[] {
  ...
  const blocks: number[] = [];
  for (let i = 0; i < 40; i += 2) blocks.push((bits[i] << 1) | bits[i + 1]);
  return blocks;
}

/** Animated decode visualization — walks the user through blocks →
 *  bit pairs → 40-bit number → 8-char base32 token. Plays once on mount
 *  when a real token is available; idle/illustrative when not. */
function DecodeAnimation({ token }: { token: string | null }) {
  const blocks = token ? tokenToBlocks(token) : [];
  const realToken = token && blocks.length === 20;
  const displayToken = realToken ? token : "ZSPANYRZ"; // illustrative when no real token
  const displayBlocks = realToken ? blocks : tokenToBlocks(displayToken);

  const [step, setStep] = useState(0);
  // Steps: 0=ribbon only, 1=labels revealed, 2=bits joined, 3=token reveal, 4=done
  useEffect(() => {
    const timers: number[] = [];
  ...
  return (
    <div className="bg-[#141416] border border-white/10 rounded-xl p-5 my-6">
      ...
      {/* Per-block bit labels */}
      <div
        className="flex justify-center font-mono text-[10px] text-gray-400 mb-3"
        style={{ gap: `${gap}px` }}
      >
        {displayBlocks.map((value, i) => (
          <span
      ...
      {/* Joined bits string */}
      <div
        className="font-mono text-[11px] text-gray-300 text-center mb-3 px-2"
        style={{
          opacity: step >= 2 ? 1 : 0,
      ...
      {/* Final token reveal */}
      <div
        className="font-mono text-[20px] tracking-[0.2em] text-white text-center"
        style={{
          opacity: step >= 3 ? 1 : 0,
    ...
    if (authenticated && v.source === "cli_generation") {
      return (
        ...
        >
          ...
          {v.status === "superseded" && (
            <div className="mt-3 text-[12px] text-[#F5A524]">
              Superseded — a newer registration replaced this generation.
            </div>
          )}
          {v.account_state === "deleted" && (
            <div className="mt-3 text-[12px] text-gray-400">
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      <div className="max-w-md mx-auto px-5 py-6">
        ...
        {/* Verdict (when arrived via ribbon click) */}
        {directToken && (
          <div className="mb-5">
            {directLoading ? (
              <div className="text-[13px] text-gray-400 italic">Looking up…</div>
            ) : (
              renderVerdictCard(directVerdict)
            )}
          </div>
        )}

        {/* Decode animation — always plays, gives the visual moment */}
        ...
        <button
          ...
          disabled={uploading}
          className="w-full bg-white text-black font-semibold py-3 rounded-lg text-[14px] tracking-wide disabled:bg-gray-700 disabled:text-gray-500"
        >
        ...
        {uploadResult && !uploading && (
          <div className="mt-3">
            {uploadResult.error ? (
              <div className="border border-[var(--alert-red)]/40 bg-[var(--alert-red)]/5 rounded-lg p-3 text-[12px] text-gray-300">
              ...
              renderVerdictCard(uploadResult.verdict)
            ) : (
              <div className="border border-[#F5A524]/40 bg-[#F5A524]/5 rounded-lg p-3 text-[12px] text-gray-300">
