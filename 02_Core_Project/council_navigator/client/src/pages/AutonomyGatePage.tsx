/**
 * AutonomyGatePage — S-007 orchestrator autonomy gate (dev/operator surface).
 *
 ...
 * the way you'd describe a colleague's job — not a schema field (D-054). The
 ...
 * built, S-007 step 3) reads `autonomous_enabled` per capability at each
...
// ── S-010 ingestion metering (the calibration dial + its live readout) ──
type Calibration = {
  videos_per_day: number;
  reviewers: number;
  reviews_per_reviewer_per_day: number;
  available_balance: number | null; // S-010 budget pot ($); null = unconfigured
...
type Metering = {
  ...
  ceilings: {
    ...
    budget_per_day: number | null; // S-010 budget ceiling (null = unconfigured)
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans">
      ...
      <main className="max-w-4xl mx-auto px-6 lg:px-10 py-8">
        ...
        {/* S-010 — the calibration dial + its live readout. The pace knob James
        ...
        {gov && (
          <section className="border border-[#F2A91C]/30 rounded-lg p-5 bg-[#F2A91C]/[0.04] mb-8">
            ...
            {/* S-010 budget / solvency dials. The balance is the pot the machine
                ...
                rail against runaway spend + the funded-tier ceiling (D-064). */}
            <div className="flex items-center justify-between gap-4 py-3 border-t border-[var(--line)]/60">
              <div className="min-w-0">
