...
//
// Reads /api/auth/me, which returns whichever provider-neutral account the
// request's `zspan_session` cookie identifies (or {authenticated: false} for
// anonymous visitors). Google and email/password mint the same cookie shape.
//
// Single source of truth for "who is the viewer, and are they the
// operator." Returns the account principal plus `isOwner` — true when its
// verified email matches the server's owner_email. Per V1-Polish-2
// (2026-06-14) operator gating
...
// deny-list) — but the UI's notion of "operator" is the configured owner.
//
...
export interface CurrentUser {
  ...
  // True when this signed-in account is the configured operator
  ...
  // Unverified local registration is forbidden for owner email addresses.
  is_owner: boolean;
...
export interface CurrentUserState {
  ...
  loading: boolean;
  // Server-controlled account sign-in maintenance switch. Defaults true
