...
import { saveBrowserWorkspaceEntry } from "@/lib/browserWorkspace";
...
export function ByokQueryPanel({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (user) {
        try {
          await saveBrowserWorkspaceEntry({
            id: result.runId,
            userId: user.user_id,
            meetingId,
            query: sentQuery,
          ...
          setWorkspaceSaveError(
            "Your answer is visible here, but this browser could not save it to your device workspace.",
          );
        }
      }
    } catch (e) {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 border-t border-white/5">
      ...
      <div className="p-4 border-t border-white/5 flex-shrink-0">
        ...
        {workspaceSaveError && (
          <p role="alert" className="mb-2 px-1 text-[12px] leading-snug text-amber-300/90">
            {workspaceSaveError}
          </p>
        )}
