 ...
 *
 * D-157 (neutrality output cut, session-42/43): this surface no longer
 ...
 * decentralized-consensus audit (S-131) can only verify outputs that
...
type MemberDetail = {
  ...
  county: string | null;
  state: string | null;
  // D-157: the city's own official record, from city_intelligence
  ...
  member: {
    ...
    // recon enrichment (A2 in the D-157 decision) — not on file yet.
...
interface CastMemberPanelProps {
  ...
  // lives there (owner-only per D-157). The button renders only for the
...
export default function CastMemberPanel({
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* The official record — links, not a profile (D-157). */}
        ...
        {onOpenTruthBook && (
          <OwnerOnly>
            <button
              ...
              title="Open this member's full Record — every verified quote and tracked commitment on a shared timeline (operator-only per D-157)"
            >
              Open the Record ↗
            </button>
          </OwnerOnly>
