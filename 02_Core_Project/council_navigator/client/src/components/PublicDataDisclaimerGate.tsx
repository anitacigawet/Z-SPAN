/**
 * PublicDataDisclaimerGate — S-091 C3 per-surface wrapper + shared
 * acknowledgment modal.
 *
 * Spec source-of-truth: `01_Project_Overview/S091_DISCLAIMER_COPY_LOCKED.md`.
 * Pattern adapted from `CreatorsSignupPage::StepTwo` (D-095 fleet-media
 * disclaimer karaoke).
 *
 * Architecture:
 ...
 *
 * Future scope (S-093): progressive lockout for repeat decliners —
 * fingerprint + IP correlation, threshold-based escalation. STATE 4
 * currently is bare google.com exit; S-093 will layer escalation on
 * top.
 *
 ...
 *  visitor acknowledges (2026-07-07 session-38 audit finding: the
...
export function PublicDataDisclaimerProvider({
  ...
  // a new BroadcastPage). Per S-091 LOCKED spec § State machine, the
  ...
  // disclaimer text. S-093 progressive-lockout still composes on top of the
...
function DisclaimerModal({
  ...
  // surfaceName recorded for analytics/audit (future S-093 fingerprint
    ...
    >
      <div className="relative max-w-2xl w-full max-h-[90vh] overflow-y-auto rounded-lg border border-white/15 bg-[#0A0A0A] p-8 shadow-2xl">
        {/* Operator escape hatch — the X was intended for the owner only
            (session-80 2026-07-18 shipped it unguarded; operator flagged
        ...
        {/* Header — small project mark. Version badge retired 2026-07-04
            session-30 — the DISCLAIMER_VERSION constant still keys the
        ...
        {/* STATE 1: READING — disclaimer body. Top paragraphs operator-
            ...
            revised 2026-07-04 session-30: "intentions" → "goal";
        ...
        {stage === "reading" && (() => {
          ...
          // Project and source-catalog links live on the dedicated sources
          // page so this acknowledgment stays focused on data limitations.
        ...
        {stage === "confirming" && (
          <div className="space-y-6 py-4">
            {/* "Etching…" eyebrow retired 2026-07-04 session-31 — the
