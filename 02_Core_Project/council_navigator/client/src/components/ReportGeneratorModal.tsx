/**
 * ReportGeneratorModal — S-122 Report-V0-1 (cited-report generator).
 *
 ...
 * (is_operator_search_principal), same D-145 posture as
...
export function ReportGeneratorModal({ query, onClose }: ReportGeneratorModalProps) {
  ...
  // uses; the scope machinery is shared by design (S-122).
  useEffect(() => {
    if (!query) return;
    let cancelled = false;
