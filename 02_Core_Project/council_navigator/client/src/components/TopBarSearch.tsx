...
// shipped RAG must answer (S-040), so it doubles as a legitimacy claim.
...
function humanizePlaceType(placeType: string): string {
  const labels: Record<string, string> = {
    ...
    township: "township",
    tribal_chapter: "Tribal chapter",
    tribal_jurisdiction: "Tribal government",
    unincorporated_community: "local community",
  };
  return labels[placeType] ?? placeType.replaceAll("_", " ");
}
...
interface TopBarSearchProps {
  ...
  // S-122 Report-V0-1 — owner-only cited-report generation. Sibling
      ...
      .then(d => {
        ...
        for (const st of d.states) {
          for (const c of st.counties ?? []) {
            for (const place of [...(c.sources ?? []), ...(c.cities ?? [])]) {
              flat.push({
                kind: "place",
                name: place.name,
                county: c.county,
  ...
  return (
    <div ref={wrapRef} className="relative ml-2 flex items-center">
        ...
        <div className="absolute left-0 top-full z-50 mt-2 w-[27rem] overflow-hidden rounded-lg border border-white/15 bg-[#0A0A0C] shadow-2xl">
          {q ? (
            <>
                  ...
                  {matches.map(m => (
                    ...
                    >
                      ...
                      <span className="flex-none text-[10px] uppercase tracking-wider text-white/35">
                        {`${humanizePlaceType(m.placeType)} · ${m.county}`}
                      </span>
                    </button>
                  ))}
                </div>
              ...
              {/* S-122 Report-V0-1 — owner-only cited-report generation.
      ...
      {/* Voice dictation — owner-only speech-to-text into the search
          ...
          (biometric voice-search, removed per D-147 + operator
