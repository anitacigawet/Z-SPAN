...
export interface NavigationState {
  view:
    ...
    | "scan"
    | "audit"
    | "corrections" // RR-4 — public corrections doorbell + running log (S-043 B-4)
    | "registry" // RR-5 — public registry policy + S-124 coverage listing
    | "city-desk" // S-123 — operator-preview enterprise-wrapper suite (paper theme)
    | "city-desk-demo" // S-123 — session-only try-it walkthrough (no persistence)
  ...
  params?: {
    ...
    cityName?: string;
    meetingId?: number;
    // D-164 facts-only catalog identity for unpublished placeholder pages.
    publicId?: string;
    ...
    compare?: boolean;
    legacy?: boolean;
    // statusOpen removed per D-169 with the scrape-daemon STATUS drawer.
    ...
    // Printed invitation bearer token. It lives only in the path and is
    // never converted to a query parameter or persisted by the client.
    inviteToken?: string;
    // Provider-neutral account doorway. Tokens stay URL-scoped and are never
...
const VALID_VIEWS: Array<NavigationState["view"]> = [
  ...
  "following", "invite", "login", "creators", "watermark-debug", "watermark-verify", "scan", "audit",
...
// its county/city drill-down stays within ChannelsPage. D-157 keeps Cast public
...
export function parseNavigationFromUrlParts(
  search: string,
  rawPathname: string,
  rawHash = "",
): NavigationState | null {
  const sp = new URLSearchParams(search);
  const fragment = new URLSearchParams(rawHash.replace(/^#/, ""));
  ...
  // S-102 pathname routing — bare URLs `/scan` and `/audit` map to the
  ...
  const invitationPath = pathname === "/i"
    && invitationToken
    && /^[A-Za-z0-9_-]{24,64}$/.test(invitationToken);
  const invitationResume = pathname === "/i"
    && fragment.get("resume") === "invitation";
  ...
  const pathView = invitationPath || invitationResume
    ? "invite"
    : PATH_VIEWS[pathname];
  const requestedView = (sp.get("view") || "").trim();
  // Home-directory URLs intentionally omit `view=home`. Treat their
  // geography parameters as an implicit home view so the URL written by
  // buildUrlForNavigation survives reload and browser history navigation.
  const hasHomeDirectoryParams = [
    "state", "countyName", "county", "cityName", "city",
  ].some(key => sp.has(key));
  const raw = pathView ?? (requestedView || (hasHomeDirectoryParams ? "home" : ""));
  ...
  if (invitationPath && invitationToken) params.inviteToken = invitationToken;
  if (view === "login") {
    const invite = fragment.get("invite");
    if (invite && /^[A-Za-z0-9_-]{24,64}$/.test(invite)) {
      params.inviteToken = invite;
    }
    const reset = fragment.get("reset");
    if (reset && /^[A-Za-z0-9_-]{32,128}$/.test(reset)) {
      params.resetToken = reset;
    }
    const next = fragment.get("next") || sp.get("next");
    if (next && next.length <= 2048) params.nextPath = next;
  }
  const stateParam = sp.get("state");
  if (stateParam) params.state = stateParam;
  const countyName = sp.get("countyName") || sp.get("county");
  if (countyName) params.countyName = countyName;
  ...
  // S-122 Report-V0-1 — ?t=<seconds> seek param (YouTube convention) so
  ...
  // retired per D-169 with the scrape-daemon STATUS drawer.)
...
function parseInitialNavigationFromUrl(): NavigationState | null {
  ...
  const parsed = parseNavigationFromUrlParts(
    window.location.search,
    window.location.pathname || "",
    window.location.hash || "",
  );
  if (parsed?.view !== "invite" || parsed.params?.inviteToken) return parsed;
  try {
    const pending = window.sessionStorage.getItem(
      PENDING_INVITATION_STORAGE_KEY,
    );
    window.sessionStorage.removeItem(PENDING_INVITATION_STORAGE_KEY);
    if (pending && /^[A-Za-z0-9_-]{24,64}$/.test(pending)) {
      return { view: "invite", params: { inviteToken: pending } };
    }
  } catch {
    // Storage can be unavailable in hardened browsers. Without the bearer,
    // the resume route must not pretend an invitation was recovered.
  }
  return null;
}

// A2 / D-148-era shareability (2026-07-01): the exact inverse of
...
export function buildUrlForNavigation(nav: NavigationState): string {
  ...
  if (p) {
    ...
    // S-122 — seek serializes as ?t= so a chip-followed moment is itself
    ...
    if (nav.view === "login" && p.inviteToken) sp.set("invite", p.inviteToken);
    if (nav.view === "login" && p.resetToken) sp.set("reset", p.resetToken);
    if (nav.view === "login" && p.nextPath) sp.set("next", p.nextPath);
  ...
  if (nav.view === "invite" && p?.inviteToken) {
    sp.delete("view");
    const inviteQuery = sp.toString();
    const suffix = inviteQuery ? `?${inviteQuery}` : "";
    return `/i${suffix}#invite=${encodeURIComponent(p.inviteToken)}`;
  }
  if (nav.view === "login") {
    sp.delete("view");
    const publicQuery = new URLSearchParams(sp);
    publicQuery.delete("invite");
    publicQuery.delete("reset");
    publicQuery.delete("next");
    const fragment = new URLSearchParams();
    if (p?.inviteToken) fragment.set("invite", p.inviteToken);
    if (p?.resetToken) fragment.set("reset", p.resetToken);
    if (p?.nextPath) fragment.set("next", p.nextPath);
    const query = publicQuery.toString();
    const sensitive = fragment.toString();
    return `/login${query ? `?${query}` : ""}${sensitive ? `#${sensitive}` : ""}`;
  }
  return qs ? `/?${qs}` : "/";
}
...
function App() {
  ...
  // S-122 Report-V0-1 — cited-report generation. Same open/closed-state
  ...
  const OWNER_ONLY_VIEWS: Array<NavigationState["view"]> = [
    ...
    "dashboard",
    "city-dashboard",
    // "following" moved to PUBLIC_VIEWS in session-103 (product-slice2) —
    ...
    // and the D-095 Creator Network signup landing get owner-only visibility
    ...
    // The HQ is the S-006 public lobby — built to be looked at. Its one write
    ...
    // client nav entry point, and its session-31 reasoning holds.
    "speaker-roster-review",
  ];
  const ownerView =
  ...
  // than OWNER_ONLY: any signed-in account passes, but
  ...
  //
  // The semantic test is whether the route is meaningless without an
  // account. Workspace meets it because every local record and receipt is
  // scoped to the current Google user. Other candidates ruled out:
  ...
  //     session-29 pre-launch gate (higher tier, not this one).
  ...
  const SIGNED_IN_ONLY_VIEWS: Array<NavigationState["view"]> = ["workspace"];
  ...
  // S-091 disclaimer-gate scopeKey: ONLY defined for episode pages
  ...
  return (
    <ThemeProvider defaultTheme="dark" storageKey="zspan-theme">
      ...
      <div className="min-h-screen bg-background text-foreground">
        ...
        {navigation.view !== "invite" && navigation.view !== "login" && <HandTrackingToggle />}
        <div
          ref={contentRef}
        ...
        >
          {ownerGateLoading || signedInGateLoading ? (
            ...
            // signed-in-only view (such as Workspace) via
            ...
            <>
              ...
              {navigation.view === "home" && (
                <ChannelsPage
                  onNavigate={navigate}
                  selectState={navigation.params?.state}
              ...
              {navigation.view === "invite" && navigation.params?.inviteToken && (
                <InvitePage
                  token={navigation.params.inviteToken}
                  onNavigate={navigate}
                />
              )}
              {navigation.view === "login" && (
                <LoginPage
                  invitationToken={navigation.params?.inviteToken}
                  resetToken={navigation.params?.resetToken}
                  nextPath={navigation.params?.nextPath}
                  onNavigate={navigate}
                />
              )}
              ...
              {/* S-123 City Desk — operator-preview enterprise-wrapper
              ...
              {/* S-123 iteration 3 — the try-it walkthrough. Session-only
        ...
        {/* S-122 Report-V0-1 — same floating-above-all-views placement and
