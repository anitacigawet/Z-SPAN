 ...
 * the YouTube Data API matcher in T-004).
 */
async function proxyToFlask(
  ...
  options: {
    ...
    body?: any;
    timeoutMs?: number;
    // S-004 — when set, the X-Zspan-Agent-Role header on the incoming
...
async function startServer() {
  ...
  app.use((req, res, next) => {
    // A complete private contribution can contain a long word-timestamp
    // transcript plus four rendered outputs. Flask independently enforces
    // the same 25 MiB ceiling and validates every nested field and hash.
    if (req.method === "POST" && req.path === "/api/contributions/submit") {
      return contributionJsonParser(req, res, next);
    }
  ...
  //
  // BRA-2 (D-180): Railway probes /healthz without an edge token. Keep this
  ...
  app.use("/media", express.static(mediaRoot, {
    ...
    setHeaders: (res) => {
      ...
      // gemini.google.com for the T-013 V2 verification step.
  ...
  // ============================================
  // D-164 public /v1 catalog — anonymous, status-preserving parity with Flask.
  ...
  // BRA-A (D-180): the public Pages surface terminates at this Express
  ...
  app.get("/public-api/channels/tree", (req, res) =>
    proxyJsonAnonymous(
      req,
      res,
      withPublicQuery("/public-api/channels/tree", { state: req.query.state }),
    ),
  );
  app.get("/public-api/catalog/contribute/:sourceId.md", (req, res) =>
    proxyJsonAnonymous(
      req,
      res,
      withPublicQuery(
        `/public-api/catalog/contribute/${encodeURIComponent(req.params.sourceId)}.md`,
  ...
  // S-067 (2026-06-19) — universal city coordinate lookup via Census
  ...
  // D-131 for the structural-spec-plus-selection-discipline pattern.)
  ...
  // surface per D-126 §5 + S-071. Pure Qdrant retrieval filtered by member
  ...
  //
  // Per D-133 + BYOK_ARCHITECTURE_SPEC: the browser calls these paths and
  ...
  app.post("/api/byok/relay", express.json({ limit: "1mb" }), async (req, res) => {
    try {
      ...
      try {
        // D-145 (2026-07-01): forward the session cookie for the Flask
        // owner-or-public-flag gate (open-ended BYOK is V2-locked for
        // the public until the S-119 query-safety research lands).
  ...
  // for the D-145 owner-or-public-flag gate) but pipes the Flask SSE
  ...
  // Public Z-SPAN coverage status. Intentionally public; no auth is involved
  // on either side.
  ...
  // RR-4 — public corrections log (S-043 B-4). The read is intentionally
  ...
  // Owner-only mutations (gated Flask-side from birth per the S-129
  ...
  // S-098 Phase 2 — decode a watermark ribbon out of an uploaded image
  ...
  // S-098 Phase 1.5 — watermark token → notebook_outputs row lookup.
  ...
  app.post("/api/rag-search/:meetingId", express.json(), async (req, res) => {
    try {
      ...
      const timeoutId = setTimeout(() => controller.abort(), 60_000);
      try {
        // D-145 (2026-07-01): forward the session cookie so Flask's
  ...
  // S-122 Report-V0-1 — cited-report generator. Same owner-only gate as
  ...
  // Report-Stitch-1 (S-122 V0.5) — fire the generative-chrome driver.
  ...
  // D-039 aggregated status (auth + work_orders + processing) — one
  ...
  // D-039 follow-up: cross-session heartbeat. The StatusBanner posts to
  ...
  // S-129 — vestigial post-D-143, zero callers, was ungated).
  ...
  // T-004: confirm a medium/needs_review match — copies meetings.video_url
  ...
  // publish / unpublish are the D-032 human-review-gate mechanisms
  ...
  // REMOVED per D-138 (2026-06-25). Manual video-URL paste struck from
  ...
  app.post("/api/work-orders/:id/set-video-url", express.json(), async (_req, res) => {
    res.status(410).json({
      success: false,
      error: "Endpoint removed per D-138 (2026-06-25). Use parsers/scripts/haiku_match_videos.py.",
  ...
  async function proxyAuthToFlask(req: any, res: any, flaskPath: string) {
    ...
    const headers: Record<string, string> = flaskProxyHeaders(req, {
  ...
  // this trap twice in session-9 alone.
  ...
  // Provider-neutral account doorway. Registration remains invitation-only;
  // Flask owns credential verification, reset tokens, and session cookies.
  for (const route of ["register", "login", "forgot", "reset"] as const) {
    app.post(
      `/api/auth/password/${route}`,
      express.json({ limit: "16kb" }),
      (req, res) => proxyAuthToFlask(req, res, `/api/auth/password/${route}`),
    );
  }

  // Printed invitation cards use a public status check and a signed-in
  // redemption call. Flask validates and atomically consumes the bearer
  // token; Express preserves the browser session and JSON body.
  app.post("/api/invitations/status", express.json(), (req, res) =>
    proxyAuthToFlask(req, res, "/api/invitations/status"),
  );

  app.post("/api/invitations/redeem", express.json(), (req, res) =>
    proxyAuthToFlask(req, res, "/api/invitations/redeem"),
  );

  app.get("/api/invitations", (req, res) =>
    proxyAuthToFlask(req, res, "/api/invitations"),
  );

  app.post("/api/invitations/import", express.json(), (req, res) =>
    proxyAuthToFlask(req, res, "/api/invitations/import"),
  );
  ...
  app.post(
    ...
    express.json(),
    (req, res) => {
      const invitationId = Number.parseInt(
  ...
  // Notification unsubscribe (session-104 catch — Haiku visitor sweep
  ...
  app.get("/api/workspace/receipts", (req, res) =>
    proxyJsonAuth(req, res, "/api/workspace/receipts"),
  );
  ...
  // V1-Repo-1 — D-095 / D-006 repository deposit gate. Approve / reject /
  // withdraw on a repository_assets row. Cf-Access D-051 is the perimeter
  ...
  // D-100 hardening findings — ingest + history list. The ingest payload
  ...
  // (Parser scraper daemon retired per D-169 — the /api/calendar/pipeline/*
  ...
  // ============================================
  // Orchestrator autonomy gate (S-007) — the dev/operator control surface
  ...
  // ============================================
  // S-010 ingestion governor — the low-hum machine's metering board.
  ...
  // ============================================
  // S-015 the Guide — present-tense directory of live civic broadcasts.
  ...
  // isOwner; Flask itself doesn't gate, D-051 edge gates in prod).
  // ============================================
  ...
  // ── Cast page V1 (T-007) — passthrough to Flask ──────────────────
  ...
  // T-012 Tracked Claims Ledger — per-city list view.
  ...
  // Truth Book Lite (D-059 Layer 1) — per-member research surface.
  ...
  // Conversational Compiler V0 (S-023 Track A) — Hex-Rays UX consumer.
  ...
  // T-012 — operator-triggered status flip on one tracked claim row.
  ...
  // T-013 V4 — operator-terminal CLI wrappers: build review queue +
  ...
  // S-006 / S-007 — HQ lobby status payload. Aggregates badges + governor +
  ...
  // S-004 — agent escalation surface. Lists unacknowledged escalations
  ...
  // T-013 V4 — source-cache cleanup. The bulky per-meeting source.mp4
  ...
  app.post("/api/work-orders/:id/clear-source-cache", express.json(), async (req, res) => {
    try {
      // RR-8 fix-list (S-129) — { req } forwards the owner session cookie;
  ...
  // T-018 — Vocabulary Inbox: list pending promotions + promote/reject one.
  ...
  // D-057 — agent counter-proposal. RR-8 SEC-AUTH: owner-OR-agent-token on
  ...
  // T-013 V4 — disputed-quotes resolution: list + resolve. Forwards the
  // S-004 agent-role header (req) so server-side log lines distinguish
  ...
  // D-057 extension — Disputed Quotes Reviewer counter-proposal. Mirrors the
  ...
  // T-013 — operator-side affordance: open the meeting's local
  ...
  // S-129 — orphaned spend surface, zero callers). Library path unchanged.

  // T-004 video matcher — runs the YouTube channel-to-video matcher for a
  ...
  // S-074 cascade-break: commits an operator-approved subset of matches
  ...
  // ── Notebook GC (D-030) — see parsers/api_server.py for safety doc ──
