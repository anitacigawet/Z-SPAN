"""Parser for Green Valley Council events from the Tribe Events REST API."""
...
FLAGSHIP_TITLES = {"board of representatives", "board of representatives meeting"}
...
class _SourceBlocked(RuntimeError):
    """The official API returned a transport/WAF surface instead of JSON."""
...
def _read_response_text_bounded(response: Any, max_bytes: int = MAX_RESPONSE_BYTES) -> str:
    body = bytearray()
...
def _fetch_json(session: Any, url: str) -> dict[str, Any]:
    ...
    try:
        ...
        sample = text.lstrip()[:300].casefold()
        if "text/html" in content_type or sample.startswith("<!doctype html") or "window.location.reload" in sample:
            raise _SourceBlocked(
                f"Green Valley API returned an HTML challenge at {url}: content_type={content_type!r}"
            ) from exc
        raise RuntimeError(f"Green Valley API returned malformed JSON at {url}") from exc
...
def _event_rows(payload: dict[str, Any], page: int) -> list[dict[str, Any]]:
    ...
    if not isinstance(raw_events, list):
        raise RuntimeError(
            f"Green Valley API payload drift on page {page}: events={type(raw_events).__name__}"
        )
    ...
    if dropped:
        raise RuntimeError(
            f"Green Valley API payload drift on page {page}: non-object rows={dict(dropped)}"
        )
    return events


def _fetch_events(session: Any, url: str) -> list[dict[str, Any]]:
    ...
    first_events = _event_rows(first_page, 1)
    if total == 0:
        if first_events:
            raise RuntimeError(
                "Green Valley API reported total=0 while returning event rows"
            )
        logger.info("green_valley pagination explicit empty total=0")
        return []
    if total_pages < 1 or total_pages > MAX_PAGES:
        raise RuntimeError(
            f"Green Valley API pagination exceeded the {MAX_PAGES}-page request cap: {total_pages}"
        )
    if total < 0 or total > MAX_ROWS:
        raise RuntimeError(
            f"Green Valley API row count exceeded the {MAX_ROWS}-row cap: {total}"
        )
    ...
    if len(events) > MAX_ROWS:
        raise RuntimeError(
            f"Green Valley API emitted more than the {MAX_ROWS}-row cap: {len(events)}"
        )
    if len(events) != total:
        raise RuntimeError(
            f"Green Valley API pagination was incomplete: fetched={len(events)} reported_total={total}"
        )
...
def _extract_location(venue: Any, event_id: str, title: str, absence_counter: Counter[str]) -> str:
    if not isinstance(venue, dict):
        ...
        logger.warning(
            "event_id=%s title=%r field=meeting_location honest-empty "
            "reason=venue_is_not_an_object type=%s",
            event_id,
            title,
            type(venue).__name__,
        )
...
def _classify_is_governance_meeting(event: dict[str, Any]) -> tuple[bool, str]:
    ...
    venue = _venue_name(event.get("venue"))
    if title_key in FLAGSHIP_TITLES:
        return True, f"title exactly matches Green Valley Council flagship body; venue={venue or 'not listed'}"
    if "board of representatives" in title_key:
        raise RuntimeError(f"Green Valley Council flagship title vocabulary drift: {title!r}")
    return False, f"title is not the Board of Representatives; venue={venue or 'not listed'}"
...
def scrape_calendar(url: str = DEFAULT_API_URL) -> list[dict]:
    """Scrape Green Valley Council Board of Representatives meetings."""
    session = make_session()
    try:
        events = _fetch_events(session, url)
    except (RequestException, _SourceBlocked) as exc:
        logger.warning("health_empty_kind=source_blocked")
        logger.warning(
            "Green Valley Council official API blocked the neutral paced request: "
    ...
    for event in events:
        ...
        meeting_date, _meeting_time = _parse_start(event.get("start_date"), event_id, title)
        if not meeting_date:
            raise RuntimeError(
                f"Green Valley API row lacks a trustworthy start_date: event_id={event_id!r} title={title!r}"
            )
        if date.fromisoformat(meeting_date) < cutoff:
            exclusion_reasons["before_current_month"] += 1
            logger.info(
                "event_id=%s title=%r excluded: date=%s before cutoff=%s",
                event_id,
                title,
                meeting_date,
                cutoff.isoformat(),
        ...
        _validate_meeting(meeting, event_id)
    ...
    if not meetings:
        ...
        logger.warning(
            "Green Valley Council official API contained no Board of Representatives rows from %s forward",
            cutoff.isoformat(),
        )
    return meetings


def _validate_meeting(meeting: dict[str, str], event_id: str) -> None:
    if tuple(meeting) != CANONICAL_FIELDS:
        raise RuntimeError(f"Green Valley schema drift for event_id={event_id}: {tuple(meeting)!r}")
    for field, value in meeting.items():
        if not isinstance(value, str):
            raise TypeError(
                f"Green Valley field must be str: event_id={event_id} field={field} "
    ...
    for field in ("agenda_url", "minutes_url", "video_url", "agenda_packet_url", "ecomment_url"):
        if meeting[field] and not meeting[field].startswith("https://"):
            raise RuntimeError(
                f"Green Valley emitted a non-HTTPS URL: event_id={event_id} field={field}"
            )
