...
PAGE_LIMIT = 20
EVENT_LIMIT = 200
COUNCIL_TITLE_RE = re.compile(r"^Regular City Council Meeting$", re.IGNORECASE)
RECOGNIZED_OTHER_BODY_RE = re.compile(
    r"\b(?:board|commission|committee|authority|district)\b",
    re.IGNORECASE,
)
...
def _api_url_from_input(url: str) -> str:
    ...
    if host.endswith(".api.civicclerk.com") and parsed.path.rstrip("/").endswith("/v1/Events"):
        if parsed.scheme != "https" or host != "surpriseaz.api.civicclerk.com":
            raise ValueError(f"Surprise API URL is not the approved HTTPS tenant: {url!r}")
        return urljoin(f"{parsed.scheme}://{host}", parsed.path.rstrip("/"))
    if host.endswith(".portal.civicclerk.com"):
        if parsed.scheme != "https" or host != "surpriseaz.portal.civicclerk.com":
            raise ValueError(f"Surprise portal URL is not the approved HTTPS tenant: {url!r}")
        tenant = host.split(".", 1)[0]
        return f"https://{tenant}.api.civicclerk.com/v1/Events"
    raise ValueError(f"Surprise input URL is not its approved CivicClerk portal/API: {url!r}")


def _load_events(session: object, api_url: str) -> tuple[list[dict[str, Any]], bool]:
    ...
    while next_url and len(events) < EVENT_LIMIT:
        ...
        params = None
    if next_url and len(events) >= EVENT_LIMIT:
        raise ValueError(
            f"Surprise CivicClerk event cap {EVENT_LIMIT} reached before pagination ended"
        )
    return events, False
...
def scrape_calendar(url: str) -> list[dict]:
    ...
    with make_session() as session:
        events, source_blocked = _load_events(session, api_url)
    if source_blocked:
        return []
    trail = Trail()
    ...
    for event in events:
        ...
        if date.fromisoformat(meeting_date) < month_floor:
            trail.dropped_row(
                ...
                f"meeting_date={meeting_date} floor={month_floor.isoformat()}",
            )
            logger.warning(
                "row=%s dropped before current month meeting_date=%s floor=%s title=%r",
                row_id,
                meeting_date,
            ...
            continue
        if not COUNCIL_TITLE_RE.fullmatch(title):
            if RECOGNIZED_OTHER_BODY_RE.search(title):
                trail.dropped_row(row_id, "recognized-other-body", title)
                logger.warning(
                    "row=%s dropped as recognized non-council body title=%r",
                    row_id,
                    title,
                )
                continue
            ambiguous_titles.append(title)
            trail.dropped_row(row_id, "ambiguous-current-title", title)
            logger.warning(
                "row=%s dropped as ambiguous current governing-body title=%r",
                row_id,
                title,
            )
            continue
        ...
        logger.info(
            "row=%s accepted meeting_date=%s title=%r status=%s",
            row_id,
            meeting_date,
            title,
            status,
    ...
    if ambiguous_titles:
        raise ValueError(
            "Surprise current source exposed unrecognized meeting titles: "
            f"{sorted(set(ambiguous_titles))!r}"
        )
    if not rows:
        logger.warning("health_empty_kind=confirmed_empty")
        logger.warning(
            "Surprise official CivicClerk API was accessible but exposed no exact "
            "Regular City Council Meeting from %s forward",
            month_floor.isoformat(),
        )
    return rows
