"""City of Goodyear council calendar parser.

Goodyear's current calendar surface is the city .gov page. The older
goodyear.legistar.com instance is a stale archive and is deliberately not used.
"""
...
_DEFAULT_URL = (
    "https://www.goodyearaz.gov/government/advanced-components/advanced-components/"
...
ALLOWED_HOSTS = {
    "goodyearaz.gov",
    "www.goodyearaz.gov",
    # Recon verified current Goodyear recordings live on OpenMedia.
...
# Goodyear's Cloudflare endpoint currently sends a cross-signed chain whose
# final public CA is absent from the worker's certifi bundle.  This exact CA is
# published by SSL.com at:
# https://ssl.com/repo/certs/SSL.com-TLS-T-ECC-R2.pem
# SHA-256: 5D:1B:C3:99:27:4E:64:9E:1C:72:69:7D:E9:1A:54:AD:
#             72:50:88:C5:22:1C:B6:1E:17:EE:9C:29:0B:C4:2A:92
# It augments the normal trust store; hostname/signature/expiry verification
# remains enabled.  Never replace this with verify=False.
_SSL_COM_TLS_TRANSIT_ECC_CA_R2_PEM = """-----BEGIN CERTIFICATE-----
...
def scrape_calendar(url: str) -> list[dict]:
    ...
    stats = _Stats()
    session = make_session(additional_ca_pem=_SSL_COM_TLS_TRANSIT_ECC_CA_R2_PEM)
    ...
    try:
        ...
        logger.warning("Goodyear official calendar failed verified TLS")
        _log_summary(stats)
        return []
    ...
    if not meetings:
        ...
        logger.warning(
            "Goodyear witnessed zero current-month-forward City Council rows on the accessible official calendar"
        )
    return meetings


def _fetch_text_bounded(session, url: str) -> _FetchResult:
...
def _detect_blocker(fetch: _FetchResult) -> tuple[str, dict[str, str | int | bool]]:
    ...
    if fetch.status_code in {401, 403, 429}:
        return f"http_{fetch.status_code}_access_block", evidence
...
def _parse_meetings(soup: BeautifulSoup, base_url: str, stats: _Stats) -> list[dict]:
    ...
    if not rows:
        if not _explicit_empty_notice(soup):
            raise ValueError(
                "Goodyear accessible calendar had no candidate rows and no explicit empty-state witness"
            )
        logger.warning(
            "Goodyear accessible page explicitly witnessed zero current meeting rows"
        )
    ...
    for row_index, row in enumerate(rows, start=1):
        ...
        if meeting_date < current_month:
            stats.drop("before_current_calendar_month")
            logger.info(
                "Goodyear row dropped: reason=before_current_calendar_month date=%s cutoff=%s evidence=%s",
                meeting_date,
                current_month,
                evidence,
            )
        ...
        if not _is_council_meeting_title(meeting_title):
            stats.drop("not_city_council_meeting")
            logger.info(
                "Goodyear row dropped: reason=not_city_council_meeting title=%r evidence=%s",
                meeting_title,
                evidence,
            )
            continue
    ...
    if not meetings and (
        stats.drop_reasons["missing_or_ambiguous_date"]
        or stats.drop_reasons["missing_title"]
    ):
        raise ValueError(
            "Goodyear candidate rows contained malformed date/title evidence, so an official zero cannot be witnessed"
        )
    return meetings


def _explicit_empty_notice(soup: BeautifulSoup) -> bool:
...
def _is_council_meeting_title(text: str) -> bool:
    lowered = _word_text(text)
    return "city council" in lowered and ("meeting" in lowered or "session" in lowered)
