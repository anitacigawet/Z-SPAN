...
DEFAULT_URL = urljoin(BASE_URL, "Meetings/Search?dropid=3")
ALLOWED_HOSTS = {"tucsonaz.hylandcloud.com"}
...
COUNCIL_RE = re.compile(r"\bmayor\s*&\s*council\b", re.IGNORECASE)


def scrape_calendar(url: str | None = None) -> list[dict[str, str]]:
    """Return current-month-forward Tucson Mayor & Council OnBase rows."""
    ...
    if "Meeting Search Results" not in page_title:
        ...
        raise RuntimeError("Tucson OnBase fingerprint drifted")
    if not meeting_rows and not re.search(r"Showing\s+0\s+Meeting", page_text, re.IGNORECASE):
        ...
        raise RuntimeError("Tucson OnBase row structure drifted")
    logger.info(
        "vendor_fingerprint witness=OnBase_Meeting_Search_Results_plus_meeting_rows rows=%d",
        len(meeting_rows),
    )
    logger.warning(
        "field_absence fields=meeting_location,video_url,ecomment_url "
        "reason=tucson_search_rows_expose_no_same_row_signal"
    )
    ...
    for row in meeting_rows:
        ...
        if title_cell is None and cells:
            title_cell = cells[0]
        if time_cell is None and len(cells) > 2:
            time_cell = cells[2]
        ...
        if not COUNCIL_RE.search(title):
            non_council += 1
            logger.info("drop_row reason=non_mayor_and_council title=%r", title)
            continue
        ...
        packet_url = _row_url(
            row,
            ("lnkMeetingAgendaDoc_", "lnkAgendaPacket_"),
            target,
            "agenda_packet_url",
            title,
        )
        minutes_url = _row_url(
            row,
            ("lnkMinutes_", "lnkMeetingSummary_", "lnkMinutesPacket_"),
            target,
            "minutes_url",
            title,
        )
...
def _search_url(url: str) -> str:
    ...
    if parsed.scheme != "https" or _host(url) not in ALLOWED_HOSTS:
        raise ValueError("Tucson source URL must use HTTPS on its official OnBase host")
...
def _fetch_text_bounded(session: requests.Session, url: str) -> str:
    with session.get(url, timeout=30, stream=True, allow_redirects=True) as response:
        if _host(response.url) not in ALLOWED_HOSTS:
            raise ValueError(f"Tucson redirect reached disallowed host: {_host(response.url)}")
        body = bytearray()
        for chunk in response.iter_content(64 * 1024):
            body.extend(chunk)
            if len(body) > MAX_RESPONSE_BYTES:
                raise ValueError(f"Tucson response exceeded {MAX_RESPONSE_BYTES} bytes")
...
def _assert_schema(rows: list[dict[str, str]]) -> None:
    for index, row in enumerate(rows):
        if tuple(row) != CANONICAL_FIELDS or any(not isinstance(value, str) for value in row.values()):
            raise ValueError(f"Tucson row {index} violates canonical schema")
