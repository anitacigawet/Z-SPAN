"""Scraper for Huachuca City council agenda/minutes PDFs."""
...
class SourceBlocked(RuntimeError):
    """The official council document surface could not be read politely."""
...
def scrape_calendar(calendar_url: str) -> list[dict]:
    ...
    year_url = _build_year_url(calendar_url, month_floor.year)
    logger.info("computed_year=%s built_url=%s", month_floor.year, year_url)
    ...
    try:
        ...
        logger.warning(
            "Huachuca City official council directory was blocked: "
    ...
    required_fingerprint = {
        "class:m1downloadlist-or-m1dll",
        "url-param:m1dll_index_get",
    }
    if not required_fingerprint.issubset(fingerprint_tokens):
        raise ValueError(
            "Huachuca City council directory fingerprint changed: "
            f"observed={fingerprint_tokens!r} required={sorted(required_fingerprint)!r}"
        )
    logger.info("vendor fingerprint confirmed tokens=%s", ", ".join(fingerprint_tokens))

    rows, rows_dropped = _extract_rows_from_soup(soup, final_url, month_floor)
    logger.info("scrape complete rows_emitted=%s rows_dropped=%s", len(rows), rows_dropped)
    if not rows:
        logger.warning("health_empty_kind=confirmed_empty")
        logger.warning(
            "Huachuca City official current-year council directory was accessible "
            "but had no meeting documents from %s forward",
...
def _build_year_url(calendar_url: str, year: int) -> str:
    ...
    if parsed.scheme != "https" or host not in ALLOWED_HOSTS:
        raise ValueError(f"Huachuca City calendar URL is not approved HTTPS source: {calendar_url!r}")
...
def _fetch_text_bounded(session: object, url: str) -> tuple[str, str]:
    if not url:
        logger.warning("fetch skipped because url is empty")
        raise ValueError("Huachuca City fetch URL is empty")
    try:
        with session.get(url, timeout=30, stream=True, allow_redirects=True) as response:
            ...
            if final_host not in ALLOWED_HOSTS:
                raise ValueError(
                    f"Huachuca City redirect to disallowed host: {final_host!r} "
                    f"started={url!r} final={response.url!r}"
                )
            ...
            for chunk in response.iter_content(chunk_size=64 * 1024):
                ...
                if len(body) > MAX_CALENDAR_BYTES:
                    raise SourceBlocked(
                        f"response from {response.url} exceeded {MAX_CALENDAR_BYTES} bytes"
                    )
            return bytes(body).decode(response.encoding or "utf-8", errors="replace"), response.url
    except RequestException as exc:
        raise SourceBlocked(f"fetch failed url={url}: {exc}") from exc
...
def _extract_rows_from_soup(
    ...
    for anchor in anchors:
        ...
        if meeting_date == "":
            ...
            continue
        effective_floor = month_floor or date(datetime.now().year, 1, 1)
        if date.fromisoformat(meeting_date) < effective_floor:
            rows_dropped += 1
            logger.warning(
                "row dropped row_id=%s reason=before_current_month meeting_date=%s "
                "floor=%s rows_accepted=%s rows_dropped=%s",
                row_id,
                meeting_date,
