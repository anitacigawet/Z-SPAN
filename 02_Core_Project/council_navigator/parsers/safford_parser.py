...
CALENDAR_URL = "https://www.cityofsafford.us/AgendaCenter"
ALLOWED_HOSTS = {"www.cityofsafford.us", "cityofsafford.us"}
BLOCKED_STATUSES = {401, 403, 429}
MAX_RESPONSE_BYTES = 4_000_000
COUNCIL_CATEGORY_LABEL = "Council Meeting Agendas and Minutes"
CANONICAL_FIELDS = (
    "meeting_title",
    "meeting_date",
    "meeting_time",
...
def _fetch_text(session, url: str) -> tuple[int, str, str]:
    ...
    if parsed.scheme != "https" or _hostname(url) not in ALLOWED_HOSTS:
        raise ValueError(f"Safford source URL is not allowlisted: {url!r}")
    with session.get(
        url,
...
def _validate_civicplus_surface(html: str) -> None:
    ...
    if missing:
        raise ValueError(f"Safford page no longer matches CivicPlus Agenda Center: {missing}")
    if "CivicClerk" in html or "/api/v1/Meetings" in html:
        raise ValueError("Safford page exposed CivicClerk markers; parser expects CivicPlus")


def _find_council_category_id(html: str) -> str:
    input_re = re.compile(
    ...
    for match in input_re.finditer(html):
        ...
        if COUNCIL_CATEGORY_LABEL in _clean_text(after):
            return category_id
    raise ValueError(f"Could not find CivicPlus category id for {COUNCIL_CATEGORY_LABEL!r}")


def _years_for_category(html: str, category_id: str) -> list[int]:
    years = {
        int(year)
...
def _category_section_html(html: str, category_id: str) -> str:
    marker = f'<span id="section{category_id}"'
    start = html.find(marker)
    if start == -1:
        logger.warning("category section %s not found in landing page; parsing full response", category_id)
        return html

    next_listing = html.find('<div class="listing listingCollapse', start + len(marker))
    if next_listing == -1:
        return html[start:]
    return html[start:next_listing]
...
def _media_link(links: list[object]) -> dict[str, str] | None:
    for link in _typed_links(links):
        ...
        if "media" in section.lower() and link.get("href", ""):
            return link
    return None
...
def scrape_calendar(
    ...
    *,
    ...
    if status in BLOCKED_STATUSES:
        logger.warning(
            "Safford official Agenda Center blocked the neutral paced request: status=%d url=%s",
            status,
            final_url,
        )
        ...
        return []
    if status != 200:
        raise RuntimeError(f"Safford Agenda Center returned HTTP {status}: {final_url}")
    _validate_civicplus_surface(html)
    category_id = _find_council_category_id(html)
    month_floor = (today or date.today()).replace(day=1)

    logger.info(
        "safford CivicPlus Agenda Center surface verified; category=%s month_floor=%s",
        category_id,
        month_floor.isoformat(),
    )

    parsed_meetings = _parse_rows(_category_section_html(html, category_id), url)
    if not parsed_meetings:
        raise ValueError("Safford City Council category exposed no parseable rows or witnessed empty state")
    result = _dedupe_sort(
        [
            meeting
    ...
    if not result:
        logger.warning(
            "Safford City Council category is accessible but has no rows from %s onward (category=%s)",
            month_floor.isoformat(),
            category_id,
        )
    ...
    missing_packet = sum(1 for meeting in result if meeting["agenda_packet_url"] == "")
    missing_video = sum(1 for meeting in result if meeting["video_url"] == "")
    missing_ecomment = sum(1 for meeting in result if meeting["ecomment_url"] == "")
    if missing_time:
        logger.warning(
            "field_absence meeting_time: %s/%s rows; CivicPlus row markup exposes posting timestamps, not meeting times",
            missing_time,
            len(result),
        )
    if missing_location:
    ...
    if missing_packet:
        logger.warning(
            "field_absence agenda_packet_url: %s/%s rows; no per-row packet link in Agenda Center table",
            missing_packet,
            len(result),
        )
    if missing_video:
        logger.warning(
            "field_absence video_url: %s/%s rows; no per-row media link in Agenda Center table",
            missing_video,
            len(result),
        )
    if missing_ecomment:
