...
# Tested during generation against: "5:30 a.m.", "5:30 p.m.", "5:30am",
...
def scrape_calendar(calendar_url: str) -> list[dict]:
    ...
    if len(items) > MAX_RSS_ITEMS:
        raise RuntimeError(
            f"Kingman RSS exceeded the {MAX_RSS_ITEMS}-item safety cap: {len(items)}"
        )
    ...
    for index, item in enumerate(items, start=1):
        ...
        if meeting_day < cutoff:
            stats.drop("before_current_month")
            logger.info(
        ...
        if body_decision == "subordinate":
            stats.drop("known_subordinate_body")
            logger.info(
        ...
        if body_decision != "council":
            raise RuntimeError(
                f"Kingman current governing-body vocabulary drift at {row_label}: "
    ...
    if not meetings:
        ...
        logger.warning(
            "Kingman official RSS contained no City Council rows from %s forward",
            cutoff.isoformat(),
        )
    _log_summary(stats)
    return meetings
...
def _city_council_title_decision(title: str) -> str:
    ...
    if folded == "city council" or folded.startswith("city council "):
        return "council"
    subordinate_terms = (
        "board",
        "commission",
...
def _fetch_text_bounded(session: Any, url: str, max_bytes: int = MAX_RSS_BYTES) -> str:
...
def _validate_granicus_fingerprint(root: ET.Element, raw_xml: str, source_url: str) -> None:
    ...
    witnessed = {
        ...
        "rss_root": root.tag.casefold().endswith("rss"),
        "rss_channel": root.find("./channel") is not None,
        "xmlns_gran": "xmlns:gran" in raw_xml,
    }
    if items:
        witnessed.update(
            {
                "gran_pubDateParts": any(
                    item.find(f"{{{GRANICUS_NS}}}pubDateParts") is not None
                    for item in items
                ),
                "item_title": items[0].find("title") is not None,
                "item_description": items[0].find("description") is not None,
            }
        )
...
def _parse_companion_minutes_feed(session: Any, source_url: str, stats: _Stats) -> dict[str, str]:
    ...
    if len(items) > MAX_RSS_ITEMS:
        raise RuntimeError(
            f"Kingman companion RSS exceeded the {MAX_RSS_ITEMS}-item safety cap: {len(items)}"
        )
