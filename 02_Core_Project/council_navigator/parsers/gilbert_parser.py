...
ALLOWED_COUNCIL_TYPES = {
    "Council Regular/Special Meeting",
    "Council Study Session",
}
RECOGNIZED_NON_FLAGSHIP_TYPES = {"Council Subcommittee"}
BAD_URL_SCHEMES = (
    "javascript:",
    "data:",
    "vbscript:",
...
def scrape_calendar(url: str) -> list[dict]:
    source_url = (url or BASE_URL).strip()
    parsed_source = urlparse(source_url)
    if parsed_source.scheme != "https" or (parsed_source.hostname or "").lower() not in ALLOWED_HOSTS:
        raise ValueError(f"Gilbert source URL is not the approved HTTPS host: {source_url!r}")
    ...
    try:
        ...
        if status in {401, 403, 429}:
            ...
            logger.warning(
                "Gilbert official Hyland source blocked the bounded polite request: "
                "url=%s status=%s failure_shape=honest-empty "
                "missing_scope=current_month_forward_town_council",
                source_url,
                status,
                exc_info=True,
            )
        ...
        logger.warning("health_empty_kind=source_blocked")
        logger.warning(
            "Gilbert official Hyland source could not be reached: "
            "failure_shape=honest-empty missing_scope=current_month_forward_town_council "
            "error=%s",
            exc,
        )
        return []

    if not _validate_vendor_fingerprint(html, source_url):
        raise ValueError("Gilbert Hyland vendor fingerprint changed")

    initial_results = _extract_json_argument(html)
    if initial_results is None:
        raise ValueError("Gilbert Hyland SearchResults JSON disappeared")

    counters: Counter[str] = Counter()
    source_meetings = list(initial_results.get("Meetings") or [])
    if len(source_meetings) > MAX_ROWS:
        raise ValueError(f"Gilbert initial result set exceeded {MAX_ROWS} rows")

    parsed_meetings: list[dict[str, str]] = []
    rows_seen = 0
    rows_dropped = 0
    drop_reasons: Counter[str] = Counter()
    ambiguous_council_types: list[str] = []
    ...
    for meeting in source_meetings:
        ...
        meeting_id = _clean_text(meeting.get("ID"))
        meeting_type = _clean_text(meeting.get("MeetingTypeName"))
        if meeting_type not in ALLOWED_COUNCIL_TYPES:
            if meeting_type in RECOGNIZED_NON_FLAGSHIP_TYPES:
                rows_dropped += 1
                drop_reasons["recognized_non_flagship_body"] += 1
                logger.warning(
                    "dropped Gilbert row reason=recognized_non_flagship_body "
                    "meeting_id=%s meeting_type=%r name=%r",
                    meeting_id,
                    meeting_type,
                    meeting.get("Name"),
                )
                continue
            ambiguous_council_types.append(meeting_type)
            rows_dropped += 1
            drop_reasons["ambiguous_meeting_type"] += 1
            logger.warning(
                "dropped Gilbert row reason=ambiguous_meeting_type meeting_id=%s "
                "meeting_type=%r name=%r",
                meeting_id,
                meeting_type,
                meeting.get("Name"),
            )
        ...
        try:
            ...
            raise RuntimeError(
                "Gilbert Hyland row extraction drift: "
                f"meeting_id={meeting_id!r} meeting_type={meeting_type!r}"
            ) from exc
        ...
        if date.fromisoformat(parsed["meeting_date"]) < month_floor:
            rows_dropped += 1
            drop_reasons["before_current_month"] += 1
            logger.warning(
                "dropped Gilbert row reason=before_current_month meeting_id=%s "
                "meeting_date=%s floor=%s title=%r",
                meeting_id,
                parsed["meeting_date"],
        ...
        logger.info(
            "accepted Gilbert row meeting_id=%s meeting_date=%s "
            "meeting_type=%r title=%r",
            meeting_id,
            parsed["meeting_date"],
            meeting_type,
            parsed["meeting_title"],
    ...
    if ambiguous_council_types:
        raise ValueError(
            "Gilbert current source exposed unrecognized meeting types: "
            f"{sorted(set(ambiguous_council_types))!r}"
        )
    ...
    if not parsed_meetings:
        ...
        logger.warning(
            "Gilbert official Hyland source was accessible but exposed no qualifying "
            "Council Regular/Special Meeting or Council Study Session from %s forward",
            month_floor.isoformat(),
        )
