...
"""orchestrator_memory_write — write/replace/delete entries in the orchestrator's memory dir.

Mirrors Anthropic's consolidate-memory skill expectation: each entry is a
markdown file with frontmatter. The MEMORY.md index is a plain markdown file
(no frontmatter) written verbatim from stdin.

D-067 wrapper pattern (same shape as D-066's state-write + action wrappers):
- Hardcoded target dir (forge-resistant — agent cannot redirect the write).
- Hardcoded role attribution (agent cannot forge a different role).
- Slug validation [a-z0-9][a-z0-9-]* (blocks path traversal).
- Type enum (observation|suggestion|insight).
- Server-stamped created_at (agent cannot forge a stale timestamp).

Usage:
    set:    python3.11 scripts/orchestrator_memory_write.py set --slug foo --type observation --description "..."  (body via stdin)
    delete: python3.11 scripts/orchestrator_memory_write.py delete --slug foo
    index:  python3.11 scripts/orchestrator_memory_write.py index  (MEMORY.md content via stdin)
"""
...
MEMORY_DIR = _REPO_ROOT / "agents" / "_orchestrator_memory"
ROLE = "orchestrator"
...
def _post_to_callback(args: argparse.Namespace) -> int:
    """POST the memory-write to PC's agent relay (D-099 Phase 2.1b)."""
