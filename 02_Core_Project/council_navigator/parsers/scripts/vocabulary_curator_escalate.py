...
"""vocabulary_curator_escalate — allowlisted Slack escalation wrapper for the headless Vocabulary Curator (S-004).

Mirrors the orchestrator's escalate wrapper (D-065) with role hardcoded
to 'vocabulary-curator'. The slack_notifier per-role rate limit (5/hour
default for Opus-class judgment agents) applies automatically.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Send a Slack escalation as the Vocabulary Curator (S-004).",
    )
    parser.add_argument(
