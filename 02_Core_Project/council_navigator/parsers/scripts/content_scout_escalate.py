...
"""content_scout_escalate — allowlisted Slack escalation wrapper for the headless Content Scout (S-004).

Mirrors the orchestrator's escalate wrapper (D-065 pattern) with role hardcoded
to 'content-scout'. The slack_notifier per-role registry rate-limit (3/hour for
Sonnet-class scout, lower than Opus agents per commit 4edc5d0) applies
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Send a Slack escalation as the Content Scout (S-004 watcher).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Optional 'table.id=N' reference for already-escalated-skipped lookups.",
    )
    parser.add_argument(
