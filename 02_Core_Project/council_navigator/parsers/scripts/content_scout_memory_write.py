...
"""content_scout_memory_write — write/replace/delete entries in the Scout's memory dir.
...
Usage:
    set:    python3.11 scripts/content_scout_memory_write.py set --slug foo --type observation --description "..."
    delete: python3.11 scripts/content_scout_memory_write.py delete --slug foo
    index:  python3.11 scripts/content_scout_memory_write.py index  (MEMORY.md content via stdin)
"""
...
MEMORY_DIR = _REPO_ROOT / "agents" / "_scout_memory"
ROLE = "content-scout"
...
def main() -> int:
    ...
    # D-099 Phase 2.1b cross-machine flags (top-level so they apply to any cmd).
    parser.add_argument(
