...
"""parser_custodian_memory_write — write/replace/delete entries in the Custodian's memory dir.
...
Usage:
    set:    python3.11 scripts/parser_custodian_memory_write.py set --slug foo --type observation --description "..."
    delete: python3.11 scripts/parser_custodian_memory_write.py delete --slug foo
    index:  python3.11 scripts/parser_custodian_memory_write.py index  (MEMORY.md content via stdin)
"""
...
MEMORY_DIR = _REPO_ROOT / "agents" / "_custodian_memory"
...
def main() -> int:
    ...
    # Cross-machine callback args (D-099 Phase 2.1b). When set, POST to PC's
    # agent relay instead of writing locally. Mac claude uses these flags.
    parser.add_argument(
        "--http-callback", default=None,
        help="Optional. POST args to this URL instead of writing locally.",
    )
    parser.add_argument(
