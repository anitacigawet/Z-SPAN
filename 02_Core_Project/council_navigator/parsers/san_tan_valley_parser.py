...
COURTESY_NOTICE_RE = re.compile(r"\bcourte?sy notice\b|\bpossible quorum\b", re.IGNORECASE)
...
def _emit_url(
    ...
    """Absolutize and allowlist one same-row URL without following it."""
    raw = (href or "").strip()
    if not raw:
        return ""
...
def _row_title_from_primary(primary: object, block_text: str, row_label: str) -> str:
    ...
    before_date = re.sub(
        r"\bTown Council Meeting Regular Meeting\b",
...
def _classify_link(anchor: object, block_text: str, row_title: str) -> str:
    ...
    if host == "drive.google.com" and "support" in lowered_text and (
        "material" in lowered_text or "document" in lowered_text
    ):
...
def _meeting_id_from_links(anchors: list[object], row_label: str) -> str:
    ...
    logger.warning("meeting_id absent: row=%s reason=no_documentcenter_id", row_label)
    return ""
...
def _governing_body_disposition(source_text: str) -> str:
    """Classify only explicit same-block governing-body evidence."""
    has_council = bool(COUNCIL_RE.search(source_text))
    has_non_council = bool(NON_COUNCIL_BODY_RE.search(source_text))
    if has_council and not has_non_council:
        return "council"
    if has_non_council and not has_council:
        return "non_council"
    return "ambiguous"
...
def _build_rows_from_block(
    ...
    for primary in primaries:
        ...
        if not title or not date:
            ...
            drops += 1
            continue
        if date < current_floor:
            logger.info(
                "dropped historical meeting: title=%s date=%s current_floor=%s",
                title,
                date,
                current_floor,
            )
            drops += 1
            continue
        if COURTESY_NOTICE_RE.search(title):
            logger.warning(
                "dropped non-meeting notice: row=%s date=%s reason=courtesy_or_possible_quorum_notice",
                row_label,
                date,
            )
            drops += 1
            continue
        governing_body_evidence = " ".join((title, row_label, block_text))
        disposition = _governing_body_disposition(governing_body_evidence)
        if disposition == "non_council":
            logger.warning(
                "dropped meeting: row=%s date=%s reason=explicit_non_council_body",
                row_label,
                date,
            )
            drops += 1
        ...
        if disposition == "ambiguous":
            raise RuntimeError(
                "San Tan Valley current meeting block is governing-body ambiguous; "
                f"explicit Town/City Council evidence is required: {row_label[:300]!r}"
            )
        ...
        for anchor in row_anchors:
            ...
            href = anchor.get("href", "")
            if not field_name:
                _emit_url(href, base_url, "unclassified_url", title)
                continue
            emitted = _emit_url(href, base_url, field_name, title)
            if not emitted:
                continue
            if row[field_name]:
                logger.warning(
...
def scrape_calendar(calendar_url: str = DEFAULT_URL) -> list[dict]:
