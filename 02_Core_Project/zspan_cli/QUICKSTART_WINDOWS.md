...
`zspan open` serves your workspace at a local address like
`http://127.0.0.1:8741`. Your provider key and raw media stay off Z-SPAN.
Completed transcripts, final outputs, and audit metadata go to Z-SPAN's
private intake for review and are never published automatically. The first
run offers a one-time
