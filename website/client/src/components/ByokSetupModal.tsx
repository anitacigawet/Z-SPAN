 ...
 * shown as "Coming soon" placeholders per architecture spec § 2.4 +
...
export function LibrarianAccessGate() {
 ...
 const loginHref = `/api/auth/google/login?next=${encodeURIComponent(
 next || "/",
 )}`;
 // Chat-bubble shape mirrors the accessState="requested" bubble below so
 // the signed-out footer visually reads as the Librarian's locked-input
 // affordance the operator approved (PR #213), not a plain paragraph.
 return (
 ...
 >
 <span>
 Sign in to request Librarian access
 </span>
 </a>
 </div>
 );
...
export function ByokSetupModal({ open, onClose, onConfigured }: ByokSetupModalProps) {
 ...
 "idle" | "validating" | "valid" | "invalid" | "soft_block"
 >("idle");
 ...
 setSoftBlock({
 message: result.soft_block.message,
 email: result.soft_block.email,
 });
 setStatus("soft_block");
 return;
 }
 ...
 {status === "soft_block" && softBlock && (
 <div className="text-[13px] bg-yellow-500/10 border border-yellow-500/30 rounded-md p-3">
 <div className="text-white/90 mb-2">{softBlock.message}</div>
 <a
 href={`mailto:${softBlock.email}`}
 className="text-[#22C55E] underline text-[12px] inline-flex items-center gap-1"
 >
 {softBlock.email}
 <ExternalLink className="w-3 h-3" />
 </a>
