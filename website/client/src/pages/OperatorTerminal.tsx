...
// ConfirmDestructive import removed with the [BURN] modal
// NotebookGcPanel removed (NotebookLM removal 2026-07-01).
...
interface WorkOrder {
 ...
 county: string | null;
 // Review-gate approval. Null = not yet approved = [REVIEW] button visible.
 ...
 is_published: number;
 // match metadata. Null = no auto-match attempted; otherwise one of
...
//: filter chips read as operator vocabulary, not DB-shaped labels.
// awaiting_notebook is a HISTORICAL state — no new WOs transition to it
// post- (NotebookLM removed 2026-07-01). Kept in the label map so
// pre- WOs still render sensibly; retirement of the state itself is
// filed as.
...
// Per-chip tooltips for the FILTER row — cleanup so the operator doesn't
...
const STATE_TOOLTIP: Record<string, string> = {
 ...
 awaiting_video: "Waiting for autonomous video match — [MATCH ▸ this city] runs it now, or wait for the next haiku_match sweep. , manual paste is retired.",
 awaiting_notebook: "Legacy pre- state — historical WOs waiting for the retired NotebookLM stage. No new WOs land here post-removal.",
...
//: render a work order's state the way an operator reads it — plain
...
function humanizeWorkOrderStatus(
 ...
 // framing + [[never-frame-operator-paste-as-constraint]].
...
//: turn a raw worker/operator log line into a plain operator sentence
...
// Two-tap confirmation gate + [[preventable-harms-must-be-prevented]]
...
export default function OperatorTerminal({ onNavigate }: TerminalProps) {
 ...
 // session's email, never a hardcoded name (2026-07-09 // audit-fix: five '"James"' literals forked vocabulary against the
 ...
 // V4 — per-WO inflight-action state. Keyed by `${wo_id}:${action}`
 ...
 // all removed (2026-06-25) — manual video-URL paste is struck
 // from the project. haiku_match_videos.py is the autonomous floor.
 const [scanBusy, setScanBusy] = useState(false);
 const [matchBusy, setMatchBusy] = useState(false);
 //cascade-break — matcher results land in a review panel before
 ...
 // notebook GC panel retired (NotebookLM removal 2026-07-01).
 ...
 // Review gate state. Approval persistence is now backed by the
 ...
 // either completed or didn't. Citation-era meetings with no legacy
 ...
 // action-gating state. Non-null = the ConfirmDestructive modal is
 // [BURN] retired (NotebookLM removal 2026-07-01) — nothing to
 ...
 // Activity-log verbosity. "Curated" (default) hides the raw worker
 ...
 // Heartbeat ping follow-up) — register this tab as an active
 ...
 // internal states that carry no operator action, +
 ...
 // channel-to-video matcher trigger — RESTRUCTURED 2026-06-21 per
 //cascade-not-click. Phase 1: this fires the matcher in preview-only
 ...
 //phase 2 — commit operator-approved subset to DB. No Haiku re-fire.
 ...
 // submitVideoUrl removed (2026-06-25). Manual video-URL paste
 ...
 await fetchAll();
 if (resetOk) {
 // Pop straight into processing. makes this safe: previously-
 ...
 // burnOne + runBurn removed (NotebookLM removal 2026-07-01) —
 ...
 >
 {/* ── OTHER SESSION inline banner follow-up; Opus 2026-06-04
 ...
 color: "#A78BFA",
 tooltip:
 "Resolve disputed quotes ( V4). Each row shows Gemini's verdict; operator verifies (with optional text edit) or rejects.",
 },
 {
 ...
 color: "#F2A91C",
 tooltip:
 "Vocabulary Inbox (). Promote Gemini-surfaced corrections into the city's canonical hints JSON. Count is Kingman pilot.",
 },
 {
 ...
 color: "#22D3EE",
 tooltip:
 "Escalations Inbox. Agent-employee escalations that need operator attention.",
 },
 ].filter((p) => p.count > 0);
 ...
 role="menu"
 >
 {/* Clear-stale-notebooks entry removed
 ...
 {/* ── Matcher review panelcascade-break) ──────────
 ...
 // mode awareness: when a worker is in flight on
 ...
 // canSetUrl removed (2026-06-25). Manual video-URL
 ...
 {/* (2026-06-25): manual SET URL form
 ...
 {(
 <>
 {/* confidence pill — visible ONLY while
 ...
 it's historical noise. Color: green
 ...
 title={` match: ${wo.video_url_match_method || ""}`}
 >
 match · {wo.video_url_match_confidence}
 </span>
 )}

 {/* [CONFIRM URL] — only for awaiting_video
 ...
 title="Accept the matched URL and move WO to pending"
 >
 Confirm URL
 </button>
 )}

 {/* [SET URL] / [URL ✓] button REMOVED
 Video URLs are assigned autonomously by
 haiku_match_videos.py + parser-native capture
 +V0 transcribe_non_youtube. Confidence
 ...
 title={
 lockedTooltip ??
 "Reset to pending and start processing immediately. keeps already-completed outputs (DB + NotebookLM-side artifacts)."
 }
 >
 Retry
 </button>
 )}
 {/* [BURN] removed (NotebookLM removal
 2026-07-01). Recoverable failures now use
 [RETRY]; unrecoverable ones land as `failed`
 for operator review. */}
 {/* [BUILD] — V4: kick off build_review_queue
 ...
 {/* [INGEST] — V3 round-trip: parse every
 ...
 follow-up: two-tap confirm gate. INGEST
 ...
 : "Ingest Gemini Pro RESPONSE.md files for this meeting ( V3). Applies verdicts + text corrections, populates the vocab dictionary. Two-tap gate."
 }
 >
 {inflight
 ? "[INGESTING…]"
 ...
 before the meeting is processed.
 follow-up: two-tap confirm gate via
 ...
 : "Push this meeting's payload + media files to the flagship at zspan.org. Idempotent — re-pushing overwrites cleanly. Two-tap gate."
 }
 >
 {inflight
 ? "[PUSHING…]"
 ...
 // whether has signed off on all of
 ...
 {/* "Published" now lives in the Status column;
 ...
 className="text-[11px] uppercase tracking-widest text-left text-gray-300 hover:text-white hover:bg-white/5 px-2 py-1.5 disabled:opacity-40 disabled:cursor-wait"
 title="Ingest Gemini Pro RESPONSE.md verdicts ( V3). Applies text corrections + populates vocab dictionary. Confirms via browser dialog before firing."
 >
 ...
 className="text-[11px] uppercase tracking-widest text-left text-gray-300 hover:text-white hover:bg-white/5 px-2 py-1.5 disabled:opacity-40 disabled:cursor-wait"
 title="Push this meeting's payload + media to the flagship at zspan.org per-meeting sync). Idempotent. Confirms via browser dialog before firing."
 >
 ...
 {/* "Generating…" now lives in the Status column. */}
 </>
 )}
 </div>
 {wo.error && (
 ...
 2026-05-26). Replaces the broken ReviewGateModal per
 : the modal no longer claims to perform verification —
 V2 is the verification chain. This is the publish-moment
 ...
 log(
 "ok",
 `PUBLISH marker recorded ( verified ${publishVerifiedCount}/${publishHeroCount} hero quotes)`,
 id
 );
 ...
 publish_notes: `Published via PublishConfirmModal (WO #${id}; verified ${publishVerifiedCount}/${publishHeroCount} hero quotes)`,
 }),
 });
