 ...
 *
 * Per ACCOUNT_SYSTEM_SPEC chunk 8 + Four steps, each with a
...
// still pending Hard Preconditions); disclaimer body is
// verbatim canonical. The ElevenLabs audio render + word-timing
...
//:610-612 verbatim. Do not paraphrase — any revision goes
// through a new D-entry Two paragraphs separated by \n\n;
...
// Karaoke cadence — ms per word. With the canonical disclaimer
// (~140 words) 380 ms gives a total gate duration of ~53 seconds —
// inside's "~45-60 seconds of narration" target. When the
...
function buildSignInHref(): string {
 ...
 return "/api/auth/google/login?next=%2F%3Fview%3Dcreators";
...
export default function CreatorsSignupPage({ onNavigate }: CreatorsSignupPageProps): ReactElement {
 ...
 The Creator Network signup flow needs a Google account so the
 agreement is tied to a verifiable identity.
 </p>
 <a
