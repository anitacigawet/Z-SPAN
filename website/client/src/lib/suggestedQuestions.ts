/**
 * Standardized per-meeting-type suggested questions neutrality cut).
 *
 * re-scoped `suggested_questions` from per-meeting-GENERATED → a fixed
 ...
 * the owner's BYOK panel uses the same list as live-query seeds. Every
 * question asks about an OBJECTIVE, convergent fact (votes, money, tabled
 * items, public-comment topics, next steps) — the class the
 * decentralized-consensus audit can verify.
 *
 * ⚠️ (2026-07-31) SUPERSEDES-IN-PART's per-meeting-answer
 ...
 * `prompts/sim_query_answer.md` prompt's private-citizen guard —is
 ...
 * `transcription/sim_query_vocab.py (forthcoming)` MUST stay in sync with
 ...
 *
 * Everything else established remains in force: standardized (not
 ...
 *
 * STATUS: `claude_authored · awaits_james_review` (2026-07-08).
 * The exact wording is drafted for operator review — see
 * `prompts/ (see review ledger)`. Edit there
