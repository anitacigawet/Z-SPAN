 ...
 * - Placeholder link references (to be replaced before public ship)
 *
 * PerC3, ALL constants below are single-source-of-truth for the
 ...
 * decoupled from any repo/source release: the site launched
 * publicly while the source stays private. Null until the operator
 * flips it. Used by `formatTimeSinceLaunch` to render the *"It has
 ...
 * key embeds this version). fleet-media convention.
 */
...
export const DISCLAIMER_PLACEHOLDERS = {
 ...
 // `github` key removed 2026-07-25 honesty sweep): the repo is
 // private while open-sourcing is postponed, so the disclaimer's
 // contribute-via-GitHub affordance came down with its consumer in
 // PublicDataDisclaimerGate. Restore both at the reopening.
} as const;
