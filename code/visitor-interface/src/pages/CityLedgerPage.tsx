...
import { TrackedClaim, TrackedClaimStatus, TRACKED_CLAIM_STATUS_DISPLAY, TRACKED_CLAIM_TYPE_DISPLAY, TRACKED_CLAIM_MARKER_COLOR, formatTimeHorizon, isAgedPastHorizon, } from "../utils/trackedClaims";
...
export default function CityLedgerPage({ cityName, onBack, onNavigate, }: CityLedgerPageProps) {
    const [payload, setPayload] = useState<LedgerPayload | null>(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [statusFilter, setStatusFilter] = useState<Set<TrackedClaimStatus>>(() => new Set<TrackedClaimStatus>(["active", "unclear"]));
      ...
      <header className="sticky top-0 z-40 bg-[var(--canvas)]/95 backdrop-blur border-b border-[var(--line)]">
        <div className="max-w-7xl mx-auto px-6 lg:px-10 py-5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-5 min-w-0">
            <button onClick={onBack} className="group flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-10 py-8">
        ...
        <div className="mb-8 flex flex-wrap items-center gap-3 text-[10px] uppercase tracking-[0.18em]">
          ...
          {ALL_STATUSES.map(s => {
            ...
            return (<button key={s} onClick={() => setStatusFilter(prev => {
                    ...
                    if (on)
                        ...
                        next.add(s);
                    return next;
                })} className="px-2 py-1 border rounded-sm transition-colors" style={{
          ...
          <button onClick={() => setAgedOnly(v => !v)} className="px-2 py-1 border rounded-sm transition-colors" style={{
        ...
        {error && (<div className="border border-[var(--line)] rounded-md px-4 py-3 text-sm text-red-400">
            Could not load ledger: {error}
          </div>)}

        {!error && !loading && claims.length === 0 && (<div className="text-[13px] text-muted-foreground leading-relaxed max-w-2xl">
        ...
        <ul className="flex flex-col gap-6">
          {claims.map(c => {
            ...
            const canKaraoke = Array.isArray(c.word_timings) &&
                c.word_timings.length > 0 &&
                !!(c.video_url ?? c.meeting_video_url);
            const statusDisplay = TRACKED_CLAIM_STATUS_DISPLAY[c.status] ??
                TRACKED_CLAIM_STATUS_DISPLAY.active;
            const typeLabel = (c.claim_type && TRACKED_CLAIM_TYPE_DISPLAY[c.claim_type]) ||
                (c.claim_type ?? "Claim");
            const aged = isAgedPastHorizon(c);
            return (<li key={claimKey} className="border border-[var(--line)] rounded-md p-5 bg-[var(--surface)]/40">
                
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="min-w-0">
                    <p className="text-[14px] text-white font-semibold leading-tight">
                      {c.speaker_name ?? "Speaker"}
                      {c.speaker_role && (<span className="text-foreground/45 font-normal text-[12px] ml-2">
                          {c.speaker_role}
                        </span>)}
                    </p>
                    <button onClick={() => onNavigate("broadcast", isPublicPlane()
                    ? { publicId: c.meeting_public_id }
                    : { meetingId: c.meeting_id })} className="mt-1 text-[11px] uppercase tracking-[0.18em] text-foreground/45 hover:text-white transition-colors inline-flex items-center gap-1.5">
                  ...
                  <div className="flex flex-col items-end gap-2 flex-shrink-0">
                    <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em]">
                      <span className="px-1.5 py-0.5 border rounded-sm tabular-nums" style={{
                    ...
                    {aged && (<span className="text-[9px] px-1 py-0.5 border rounded-sm uppercase tracking-[0.18em]" style={{
                        color: "#F87171",
                ...
                {canKaraoke ? (<SyncedQuote wordTimings={c.word_timings!} videoUrl={(c.video_url ?? c.meeting_video_url)!} isActive={activeClaimId === claimKey} onActivate={() => setActiveClaimId(claimKey)} onDeactivate={() => setActiveClaimId(prev => (prev === claimKey ? null : prev))} markerColor={TRACKED_CLAIM_MARKER_COLOR}/>) : (<p className="text-[14px] text-white/85 leading-snug italic">
                    &ldquo;{c.claim_text}&rdquo;
                  </p>)}

                
                {c.expected_outcome && (<p className="text-[12px] text-foreground/60 mt-3 leading-snug">
                    <span className="text-foreground/35 uppercase tracking-[0.18em] text-[9px] mr-1.5">
                      Verifies if
                    </span>
                    {c.expected_outcome}
                  </p>)}
                {c.status_evidence && (<p className="text-[11px] text-foreground/50 mt-2 leading-snug border-l-2 border-[var(--line)] pl-3">
                    <span className="text-foreground/30 uppercase tracking-[0.18em] text-[9px] mr-1.5">
                      Status note
                    </span>
                    {c.status_evidence}
                    {c.status_updated_by && (<span className="text-foreground/30 ml-2">
                        — {c.status_updated_by}
                      </span>)}
                  </p>)}
                {c.context && (<p className="text-[11px] text-foreground/45 mt-2 leading-snug">
                    <span className="text-foreground/30 uppercase tracking-[0.18em] text-[9px] mr-1.5">
                      Context
                    </span>
                    {c.context}
