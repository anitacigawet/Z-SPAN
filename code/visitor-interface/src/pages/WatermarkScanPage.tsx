...
type Phase = "idle" | "requesting" | "scanning" | "verdict" | "error" | "debug-capturing" | "debug-results";
...
function resolvedFromVotes(votes: BlockVotes): number | null {
    const entries = Object.entries(votes).map(([k, v]) => [Number(k), v]) as [
        number,
        number
    ][];
    if (entries.length === 0)
...
export default function WatermarkScanPage() {
    ...
    const blockVotesRef = useRef<BlockVotes[]>(Array.from({ length: 20 }, () => ({} as BlockVotes)));
    const [resolvedBlocks, setResolvedBlocks] = useState<(number | null)[]>(() => Array(20).fill(null));
    ...
    const [lastFrameDims, setLastFrameDims] = useState<{
        w: number;
        h: number;
    } | null>(null);
    ...
    const startScan = async () => {
        ...
        if (!navigator.mediaDevices?.getUserMedia) {
            setErrorMsg("This browser can't open the camera. Try Safari on iOS or Chrome on Android, or upload a screenshot instead.");
            setPhase("error");
            return;
        }
        setPhase("requesting");
        ...
        try {
            ...
            if (!detected || !bbox || blocks.length !== 20) {
                ...
                if (lockLossCountRef.current >= MAX_LOCK_LOSS_FRAMES) {
                    ...
                    setLastStatus(lockState === "locked"
            ...
            if (resolvedN === 20) {
                ...
                try {
                    const lookupResp = await fetch(`/api/watermark-lookup/${encodeURIComponent(token)}`);
    ...
    const captureOneDebugFrame = async (sessionId: string, seq: number): Promise<BatchFrame | null> => {
    ...
    const renderVerdictCard = (v: Verdict | undefined | null) => {
        ...
        const background = legacy
            ...
            : authenticated
                ? "rgba(34,197,94,0.08)"
                : "rgba(239,68,68,0.08)";
        return (<div className="border rounded-lg p-4" style={{ borderColor, background }}>
        <div className="text-[16px] font-semibold mb-2" style={{
                color: legacy
                    ? "#F5A524"
                    : authenticated
                        ? "var(--success-green)"
        ...
        {legacy ? (<div className="text-[13px] text-gray-300">
          ...
          </div>) : (<div className="text-[13px] text-gray-300">
    ...
    return (<div className="min-h-screen bg-[#0A0A0A] text-white">
      <div className="max-w-md mx-auto px-5 py-6">
        ...
        {phase === "idle" && (<>
            ...
            <button onClick={() => void startScan()} className="w-full bg-white text-black font-semibold py-3 rounded-lg text-[14px] tracking-wide">
              Open camera
            </button>
            <button onClick={goToVerifyPage} className="w-full mt-2 border border-white/20 text-white font-medium py-3 rounded-lg text-[14px] tracking-wide">
            ...
            <div className="mt-8 pt-5 border-t border-white/5">
              <button onClick={goToAuditPage} className="text-[12px] text-gray-400 hover:text-white underline-offset-4 hover:underline">
                Curious how this works? Learn more →
              </button>
            </div>
            
            {import.meta.env.DEV && (<div className="mt-5">
                <button onClick={() => void startDebugCapture()} className="w-full border border-dashed border-[#F5A524]/50 text-[#F5A524] font-mono text-[11px] py-2.5 rounded-lg uppercase tracking-[0.12em]">
        ...
        {phase === "debug-capturing" && (<div className="relative rounded-xl overflow-hidden border border-[#F5A524]/40 bg-black mb-4">
            <video ref={videoRef} playsInline muted className="block w-full h-auto" style={{ maxHeight: "60vh", objectFit: "cover" }}/>
            ...
            <div className="absolute top-2 left-2 text-[11px] font-mono uppercase tracking-[0.18em] px-2 py-1 rounded" style={{
                color: "#F5A524",
        ...
        {phase === "debug-results" && (<>
            <div className="border border-[#F5A524]/30 bg-[#F5A524]/5 rounded-lg p-3 mb-4 text-[12px]">
              ...
              <div className="text-gray-400 leading-snug mt-1">
                Decoded:{" "}
                {debugFrames.filter((f) => "token" in f.result && f.result.token).length}{" "}
                / {debugFrames.length}
              </div>
            </div>
                ...
                const status = ok
                    ...
                    : found
                        ...
                        : err
                            ? `❌ error: ${err}`
                            : "⬜ no ribbon found";
                return (<div key={f.seq} className="border border-white/10 rounded-lg overflow-hidden">
                    <img src={f.preview} alt={`frame ${f.seq}`} className="block w-full h-auto"/>
                    <div className="px-3 py-2 bg-[#141416]">
                      ...
                      {"stats" in r && r.stats && (<div className="text-[10px] font-mono text-gray-500 break-all">
            ...
            <div className="flex gap-2 mt-5">
              ...
              <button onClick={() => void startDebugCapture()} className="flex-1 bg-white text-black font-semibold py-3 rounded-lg text-[14px]">
                Capture again
              </button>
            </div>
          </>)}

        {phase === "requesting" && (<div className="text-[13px] text-gray-400 italic">Asking the browser for camera access…</div>)}

        {(phase === "scanning" || phase === "verdict") && (<div className="relative rounded-xl overflow-hidden border border-white/10 bg-black mb-3">
            <video ref={videoRef} playsInline muted className="block w-full h-auto" style={{ maxHeight: "70vh", objectFit: "cover" }}/>
            <canvas ref={canvasRef} className="hidden"/>

            
            {phase === "scanning" && lockState === "searching" && (<div aria-hidden className="absolute inset-0 pointer-events-none flex items-center justify-center">
                <div style={{
                    width: "80%",
                    height: "14%",
            ...
            {(lockState === "locked" || lockState === "verifying" || phase === "verdict") &&
                ...
                const verdictAuthenticated = result?.verdict?.authenticated === true;
                const verdictLegacy = result?.verdict?.legacy === true;
                const bracketColor = phase === "verdict"
                    ? verdictLegacy
                        ? "#F5A524"
                ...
                return (<>
                    ...
                    {phase === "scanning" &&
                        resolvedBlocks.map((r, i) => {
                            ...
                            return (<div key={i} aria-hidden className="absolute pointer-events-none" style={{
                                    left: innerL,
                                    top: stripT,
                                    width: innerW,
                                    height: stripH,
                    ...
                    {phase === "scanning" && lockState === "locked" && (<div aria-hidden className="absolute pointer-events-none" style={{
                            left: scanLeft,
                            top: stripT - 3,
                            width: 2,
                            height: stripH + 6,
                    ...
                    {phase === "verdict" && result?.verdict && (<VerdictBloom bbox={{ left: bL, top: bT, width: bW, height: bH }} videoHeight={v.clientHeight} verdict={result.verdict}/>)}
                  </>);
            })()}

            
            {phase === "scanning" && (<div className="absolute top-2 left-2 text-[10px] font-mono uppercase tracking-[0.18em] px-2 py-1 rounded" style={{
                    color: lockState === "locked" || lockState === "verifying"
                        ? "#22C55E"
                        : "#F5A524",
        ...
        {phase === "verdict" && (<div className="flex gap-2">
            <button onClick={scanAgain} className="flex-1 bg-white text-black font-semibold py-3 rounded-lg text-[14px] tracking-wide">
              Scan again
            </button>
            <button onClick={goToAuditPage} className="flex-1 border border-white/20 text-white font-medium py-3 rounded-lg text-[14px]">
              Learn more
            </button>
          </div>)}

        {phase === "error" && (<>
            <div className="border border-[var(--alert-red)]/40 bg-[var(--alert-red)]/5 rounded-lg p-3 text-[13px] text-gray-200 mb-4">
              <div className="text-[var(--alert-red)] font-semibold mb-1">Camera unavailable</div>
              {errorMsg}
            </div>
            <button onClick={() => void startScan()} className="w-full bg-white text-black font-semibold py-3 rounded-lg text-[14px] tracking-wide mb-2">
              Try again
            </button>
            <button onClick={goToVerifyPage} className="w-full border border-white/20 text-white font-medium py-2.5 rounded-lg text-[13px]">
              Upload a screenshot instead
            </button>
          </>)}
      </div>
...
function VerdictBloom({ bbox, videoHeight, verdict, }: {
    bbox: {
        left: number;
        top: number;
        width: number;
    ...
    return (<>
      ...
      <div className="absolute pointer-events-none px-4 py-3 rounded-lg" style={{
            ...cardStyle,
            left: 12,
            right: 12,
            ...
            transition: "opacity 350ms cubic-bezier(0.16, 1, 0.3, 1), transform 450ms cubic-bezier(0.16, 1, 0.3, 1)",
        }}>
        <div className="text-[15px] font-semibold mb-1" style={{ color: tint }}>
          {legacy
            ? "Legacy identifier — not authentication"
            : authenticated
                ? "Registry match · canonical record"
        ...
        {legacy ? (<div className="text-[12px] text-gray-300 leading-snug">
            This is a publicly reproducible legacy identifier — not authentication.
            The screenshot content itself is not authenticated.
          </div>) : authenticated ? (<>
            ...
            {verdict.meeting_title && (<div className="text-[11px] text-gray-300 leading-snug mt-0.5">
                {verdict.meeting_title}
              </div>)}
            {verdict.output_type && (<div className="text-[10px] text-gray-500 font-mono mt-1">
                {verdict.output_type}
              </div>)}
...
function VirtualRibbon({ resolved, votes, fuzzTick, consensusToken, }: {
    ...
    const renderCell = (i: number) => {
        const r = resolved[i];
        if (r !== null) {
            return (<rect key={i} x={i * (cellW + gap)} y={0} width={cellW} height={cellH} fill={PALETTE_COLORS[r]} rx={1} ry={1}/>);
        }
        const v = votes[i] ?? {};
        ...
        return (<rect key={i} x={i * (cellW + gap)} y={0} width={cellW} height={cellH} fill={totalVotes === 0 ? "#3F3F46" : fuzzColor} fillOpacity={opacity} rx={1} ry={1}/>);
    };
    return (<div className="border border-white/10 rounded-lg p-3 bg-[#141416]">
      ...
      <div className="flex justify-center mb-2">
        <svg width={totalW} height={cellH} viewBox={`0 0 ${totalW} ${cellH}`} xmlns="http://www.w3.org/2000/svg" style={{ display: "block", maxWidth: "100%" }}>
          {Array.from({ length: 20 }, (_, i) => renderCell(i))}
        </svg>
      </div>
      {consensusToken && (<div className="font-mono text-[14px] tracking-[0.2em] text-center text-white pt-1">
          {consensusToken}
        </div>)}
    </div>);
}
