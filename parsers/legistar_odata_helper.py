"""Legistar OData API — shared helper for Legistar-platform city parsers.

The `webapi.legistar.com/v1/<jurisdiction>/events` endpoint returns
structured JSON with a stable schema. It paginates cleanly and provides
per-event metadata (EventDate, EventTime, EventBodyName, EventLocation,
EventInSiteURL, EventVideoPath, agenda + minutes file pointers). Use this
instead of scraping Calendar.aspx HTML or the Excel-export endpoint, which
can silently return HTML in place of CSV.
