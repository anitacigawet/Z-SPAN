"""Bullhead City — Granicus meeting parser."""
...
def scrape_calendar(calendar_url: str) -> list[dict]:
    ...
    for index, item in enumerate(_iter_items(root), start=1):
    ...
    if counters["rows_seen"] == 0:
        logger.warning("rss_surface_empty rows_seen=0 url=%r result=honest-empty []", calendar_url)
