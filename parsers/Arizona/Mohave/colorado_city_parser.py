...
LOCAL_SNAPSHOT = Path("/tmp/cc_page.html")
...
def scrape_calendar(calendar_url: str) -> list[dict]:
    ...
    for index, row in enumerate(meeting_list.select(":scope > li"), start=1):
...
def _load_calendar_html(calendar_url: str) -> str:
    ...
    try:
        ...
        if LOCAL_SNAPSHOT.exists() and _host(calendar_url) in ALLOWED_HOSTS:
            logger.warning(
                "calendar_fetch_failed_using_local_snapshot url=%r snapshot=%s error=%s",
                calendar_url,
                LOCAL_SNAPSHOT,
                exc,
            )
            return _read_local_snapshot_bounded(LOCAL_SNAPSHOT, MAX_CALENDAR_BYTES)
        logger.warning(
...
def _fetch_text_bounded(session: requests.Session, url: str, max_bytes: int) -> str:
...
def _read_local_snapshot_bounded(path: Path, max_bytes: int) -> str:
    size = path.stat().st_size
    if size > max_bytes:
        raise ValueError(f"Local snapshot {path} exceeded {max_bytes} bytes")
...
def _validate_custom_meetings_surface(soup: BeautifulSoup, calendar_url: str) -> Tag:
    ...
    if not rows or not rows_with_datetime:
