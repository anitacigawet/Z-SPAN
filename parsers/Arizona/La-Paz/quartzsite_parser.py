"""Quartzsite — Revize meeting parser."""
...
def scrape_calendar(url: str = DEFAULT_CALENDAR_URL) -> list[dict]:
    """Scrape upcoming Town Council meetings from Quartzsite's Revize calendar."""
    page_html = _fetch_text_bounded(url, MAX_PAGE_BYTES)
    _validate_revize_fingerprint(page_html, url)
    _validate_revize_index_js(url)
    ...
    data = json.loads(_fetch_text_bounded(endpoint_url, MAX_JSON_BYTES))
...
def _fetch_text_bounded(url: str, max_bytes: int) -> str:
    ...
    with urlopen(request, timeout=30, context=_urllib_ssl_context()) as response:  # nosec: URL host is allowlisted above.
        final_url = response.geturl()
        _validate_allowed_host(final_url, "redirect")
...
def _urllib_ssl_context() -> ssl.SSLContext:
    ...
    candidates = [Path(cafile)] if cafile else []
...
def _validate_revize_index_js(page_url: str) -> None:
    index_url = urljoin(page_url, "/revize/plugins/revize_calendar/index.js")
    index_js = _fetch_text_bounded(index_url, MAX_ASSET_BYTES)
...
def _build_meeting(
    ...
    status = _derive_status(title, description, agenda_url, agenda_packet_url)
    meeting = {
        "meeting_title": title,
...
def _derive_status(title: str, description: str, agenda_url: str, agenda_packet_url: str) -> str:
    if CANCELLED_RE.search(f"{title} {description}"[:2000]):
        return "Cancelled"
