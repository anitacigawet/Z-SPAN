...
def _build_api_url(calendar_url: str | None) -> tuple[str, str]:
    ...
    if parsed.scheme and parsed.netloc and _is_events_api_url(source_url):
    ...
    logger.warning(
        "window_rock_api_route=fallback_default input_url=%r api_url=%s reason=unparseable_input_url",
        calendar_url,
        DEFAULT_API_URL,
    )
    return DEFAULT_API_URL, _registered_domain(DEFAULT_API_URL)
...
def _fetch_text_bounded(session: object, url: str, registered_domain: str) -> str:
    ...
    with response_context as response:
        ...
        if not _host_allowed(final_host, registered_domain):
...
def _build_meeting(event: dict, api_url: str, registered_domain: str, counters: Counter) -> dict[str, str] | None:
    event_id = str(event.get("id", ""))
    ...
    agenda_url = _emit_url(event.get("url"), api_url, registered_domain, "agenda_url", event_id)
    minutes_url = ""
    video_url = ""
    agenda_packet_url = ""
    ecomment_url = ""
...
def scrape_calendar(url: str) -> list[dict]:
    ...
    try:
        ...
        logger.warning("window_rock_architectural_blocker reason=requests_unavailable error=%s", exc)
        return []
    ...
    try:
        with requests.Session() as session:
            ...
            if total_pages > MAX_TOTAL_PAGES:
                logger.warning(
                    "window_rock_architectural_blocker reason=total_pages_exceeds_cap total_pages=%d cap=%d api_url=%s",
                    total_pages,
                    MAX_TOTAL_PAGES,
                    api_url,
                )
            ...
            for page_index, page_data in enumerate(pages, start=1):
                ...
                for event in events:
                    meeting = _build_meeting(event, api_url, registered_domain, counters)
                    if meeting is None:
                        continue
