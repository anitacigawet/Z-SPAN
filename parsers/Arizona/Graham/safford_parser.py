        ...
        headers.update(
            {
                ...
                "Origin": "https://www.cityofsafford.us",
            }
        )
...
def _meeting_time_from_title(title: str) -> str:
    match = re.search(
        r"(?:@|\bat\b)\s*(\d{1,2})(?::(\d{2}))?\s*([ap])\.?\s*m\.?\b",
        title[:300],
        re.IGNORECASE,
    )
    if not match:
...
def scrape_calendar(url: str = CALENDAR_URL) -> list[dict[str, str]]:
    ...
    years = _years_for_category(html, category_id)

    logger.info(
        "safford CivicPlus Agenda Center surface verified; category=%s years=%s",
        category_id,
        years,
    )

    meetings = _parse_rows(_category_section_html(html, category_id), url)
    update_url = urljoin(url, "/AgendaCenter/UpdateCategoryList")
    for year in years:
        if any(row["meeting_date"].startswith(str(year)) for row in meetings):
            continue
    ...
    if not result:
        logger.warning(
            "Safford CivicPlus parser returned zero meetings after category=%s years=%s",
            category_id,
            years,
        )
        return result
