        ...
        headers.update(
            {
                ...
                "Origin": "https://www.douglasaz.gov",
            }
        )
...
def scrape_calendar(url: str) -> list[dict[str, str]]:
    ...
    logger.info(
        "douglas CivicPlus Agenda Center surface verified; category=%s years=%s",
        category_id,
        years,
    )
    ...
    if not result:
        raise ValueError("Douglas CivicPlus parser returned zero meetings")
