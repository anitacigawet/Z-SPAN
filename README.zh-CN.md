...
它不是生产系统的完整副本，也不是一个可以克隆后直接部署成另一个 Z-SPAN 实例的项目。这里可供借鉴的内容单元更小：一种导航思路、一条播放边界、一种让原始材料始终可见的方法，或一项可以用于其他独立项目的设计原则。
...
6. 查看[当前快照记录](docs/snapshots/2026-08-02.md)，了解本次发布的准确规模和审查状态。

## 🗂️ 书架上有什么

目前公开的源代码展示了访客体验的六个部分：
...
这个仓库没有提供安装、托管、Docker 或部署说明。这是有意为之。

这里发布的文件来自一个更大的私有工作系统，其中部分导入、服务、应用连接方式和运行配置并未公开。源代码在这里供人阅读和研究；它并不是一个独立可运行的应用，也不是受支持的软件发行版。
